...
! and output timestamp file to check that simulation is running fine
  ...
  real(kind=CUSTOM_REAL) Usolidnorm_m2,Usolidnorm_all_m2 ! elastic, ! lucas, CTD_SEM
  ...
  real(kind=CUSTOM_REAL) b_Usolidnorm_m2, b_Usolidnorm_all_m2 ! CTD_SEM
  ...
  Usolidnorm_all_m2 = 0.0_CUSTOM_REAL !lucas CTD_SEM
  ...
  if (ELASTIC_SIMULATION) then
    if (GPU_MODE) then
      ...
      if(CTD_SEM) then !------------------m2
      Usolidnorm_m2 = maxval(sqrt(displ_m2(1,:)**2 + displ_m2(2,:)**2 + displ_m2(3,:)**2))
      endif !-----------------------------m2
    endif
...
! this trick checks for NaN (Not a Number), which is not even equal to itself
    ...
    if(CTD_SEM) then !-------------------------m2
    if (displ_m2(1,1) /= displ_m2(1,1)) then
      call exit_MPI(myrank,'forward simulation became unstable in elastic domain and blew up for m2')
    endif
    ! compute the maximum of the maxima for all the slices using an MPI reduction
    call max_all_cr(Usolidnorm_m2,Usolidnorm_all_m2)
    endif !------------------------------------m2
  ...
  if (SIMULATION_TYPE == 3) then
    ...
    b_Usolidnorm_all_m2 = 0.0_CUSTOM_REAL ! CTD_SEM
    ...
    if (ELASTIC_SIMULATION) then
      ...
      if (GPU_MODE) then
        ...
        if(CTD_SEM) then !---------------------m2
        b_Usolidnorm_m2 = maxval(sqrt(b_displ_m2(1,:)**2 + b_displ_m2(2,:)**2 + b_displ_m2(3,:)**2))        
        endif !--------------------------------m2
      endif
      ! compute max of all slices
      call max_all_cr(b_Usolidnorm,b_Usolidnorm_all)
      if (CTD_SEM) then !-----------------------m2
      call max_all_cr(b_Usolidnorm_m2,b_Usolidnorm_all_m2)
      endif !-----------------------------------m2
    endif
  ...
  if (myrank == 0) then
    ...
    if (ELASTIC_SIMULATION .and. CTD_SEM) &
      write(IMAIN,*) 'Max norm displacement vector U in all slices (m) for m2 = ',Usolidnorm_all_m2 ! CTD_SEM
    ...
    if (SIMULATION_TYPE == 3) then
      ...
      if (ELASTIC_SIMULATION .and. CTD_SEM) &
        write(IMAIN,*) 'Max norm displacement vector U (backward) in all slices (m) for m2 = ',b_Usolidnorm_all_m2 ! CTD_SEM
    ...
    if (ELASTIC_SIMULATION .and. CTD_SEM) &
      write(IOUT,*) 'Max norm displacement vector U in all slices (m) for m2 = ',Usolidnorm_all_m2 ! CTD_SEM
    ...
    if (SIMULATION_TYPE == 3) then
      ...
      if (ELASTIC_SIMULATION .and. CTD_SEM) &
        write(IOUT,*) 'Max norm displacement vector U (backward) in all slices (m) for m2 = ',b_Usolidnorm_all_m2 ! CTD_SEM
...
! this trick checks for NaN (Not a Number), which is not even equal to itself
    ...
    ! CTD_SEM, elastic case only right now
    if(CTD_SEM) then !------------------------------------m2
    if (Usolidnorm_all_m2 > STABILITY_THRESHOLD .or. Usolidnorm_all_m2 < 0.0_CUSTOM_REAL .or. & 
                            Usolidnorm_all_m2 /= Usolidnorm_all_m2) &
        call exit_MPI(myrank,'forward simulation became unstable and blew up for m2')
    endif !-----------------------------------------------m2
...
! this trick checks for NaN (Not a Number), which is not even equal to itself
      ...
      if(CTD_SEM) then !------------------------------------m2
      if (b_Usolidnorm_all_m2 > STABILITY_THRESHOLD .or. b_Usolidnorm_all_m2 < 0.0_CUSTOM_REAL &
        .or. b_Usolidnorm_all_m2 /= b_Usolidnorm_all_m2) &
          call exit_MPI(myrank,'backward simulation became unstable and blew up for m2')
      endif !-----------------------------------------------m2
    endif
