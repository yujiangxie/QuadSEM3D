...
!----------------------------------- m2 -----------------------------
! CTD_SEM
subroutine compute_element_att_memory_second_order_rk_m2(ispec,alphaval_m2,betaval_m2,gammaval_m2,NSPEC_AB, & 
                          kappastore_m2,mustore_m2, &
                          ...
                          NSPEC_STRAIN_ONLY,epsilondev_xx_m2, epsilondev_yy_m2,epsilondev_xy_m2, &
                          epsilondev_xz_m2,epsilondev_yz_m2, &
                          epsilondev_xx_loc_m2,epsilondev_yy_loc_m2,epsilondev_xy_loc_m2,& 
                          epsilondev_xz_loc_m2,epsilondev_yz_loc_m2)
  ...
  integer :: ispec,NSPEC_AB,NSPEC_ATTENUATION_AB,NSPEC_STRAIN_ONLY
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: alphaval_m2,betaval_m2,gammaval_m2
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: kappastore_m2,mustore_m2
  real(kind=CUSTOM_REAL), dimension(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB) :: factor_common_m2,factor_common_kappa_m2
  real(kind=CUSTOM_REAL), dimension(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB) :: R_trace_m2,R_xx_m2,R_yy_m2,& 
                                                                                     R_xy_m2,R_xz_m2,R_yz_m2

  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY) :: &
            epsilondev_xx_m2,epsilondev_yy_m2,epsilondev_xy_m2,epsilondev_xz_m2,epsilondev_yz_m2
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY) :: epsilondev_trace_m2
...
! local parameters
  integer :: i,j,k
  real(kind=CUSTOM_REAL) :: Sn_m2,Snp1_m2
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: factor_loc_m2
  ...
  do k = 1,NGLLZ
    do j = 1,NGLLY
      do i = 1,NGLLX
        ...
        ! term in trace
        factor_loc_m2(:) = kappastore_m2(i,j,k,ispec)* factor_common_kappa_m2(:,i,j,k,ispec)

        Sn_m2   = epsilondev_trace_m2(i,j,k,ispec)
        Snp1_m2   = epsilondev_trace_loc_m2(i,j,k)
        R_trace_m2(:,i,j,k,ispec) = alphaval_m2(:) * R_trace_m2(:,i,j,k,ispec) + factor_loc_m2(:) * (betaval_m2(:) * & 
                                                   Sn_m2 + gammaval_m2(:) * Snp1_m2)

        ! term in xx yy zz xy xz yz
        factor_loc_m2(:) = mustore_m2(i,j,k,ispec) * factor_common_m2(:,i,j,k,ispec)

        ! term in xx
        Sn_m2   = epsilondev_xx_m2(i,j,k,ispec)
        Snp1_m2   = epsilondev_xx_loc_m2(i,j,k)
        R_xx_m2(:,i,j,k,ispec) = alphaval_m2(:) * R_xx_m2(:,i,j,k,ispec) + factor_loc_m2(:) * (betaval_m2(:) * & 
                                                  Sn_m2 + gammaval_m2(:) * Snp1_m2)

        ! term in yy
        Sn_m2   = epsilondev_yy_m2(i,j,k,ispec)
        Snp1_m2   = epsilondev_yy_loc_m2(i,j,k)
        R_yy_m2(:,i,j,k,ispec) = alphaval_m2(:) * R_yy_m2(:,i,j,k,ispec) + factor_loc_m2(:) * (betaval_m2(:) * & 
                                                  Sn_m2 + gammaval_m2(:) * Snp1_m2)

        ! term in zz not computed since zero trace

        ! term in xy
        Sn_m2   = epsilondev_xy_m2(i,j,k,ispec)
        Snp1_m2   = epsilondev_xy_loc_m2(i,j,k)
        R_xy_m2(:,i,j,k,ispec) = alphaval_m2(:) * R_xy_m2(:,i,j,k,ispec) + factor_loc_m2(:) * (betaval_m2(:) * & 
                                                  Sn_m2 + gammaval_m2(:) * Snp1_m2)

        ! term in xz
        Sn_m2   = epsilondev_xz_m2(i,j,k,ispec)
        Snp1_m2   = epsilondev_xz_loc_m2(i,j,k)
        R_xz_m2(:,i,j,k,ispec) = alphaval_m2(:) * R_xz_m2(:,i,j,k,ispec) + factor_loc_m2(:) * (betaval_m2(:) * &
                                                  Sn_m2 + gammaval_m2(:) * Snp1_m2)

        ! term in yz
        Sn_m2   = epsilondev_yz_m2(i,j,k,ispec)
        Snp1_m2   = epsilondev_yz_loc_m2(i,j,k)
        R_yz_m2(:,i,j,k,ispec) = alphaval_m2(:) * R_yz_m2(:,i,j,k,ispec) + factor_loc_m2(:) * (betaval_m2(:) * & 
                                                  Sn_m2 + gammaval_m2(:) * Snp1_m2)
...
end subroutine compute_element_att_memory_second_order_rk_m2


!----------------------------------- m1 -----------------------------
! CTD_SEM
subroutine compute_element_att_memory_second_order_rk_m1(ispec,alphaval,betaval,gammaval,NSPEC_AB, & 
                          kappastore,mustore, &
                          NSPEC_ATTENUATION_AB,factor_common_kappa, &
                          R_trace_m1,epsilondev_trace_m1,epsilondev_trace_loc_m1, &
                          factor_common,R_xx_m1,R_yy_m1,R_xy_m1,R_xz_m1,R_yz_m1, &
                          NSPEC_STRAIN_ONLY,epsilondev_xx_m1, epsilondev_yy_m1,epsilondev_xy_m1, &
                          epsilondev_xz_m1,epsilondev_yz_m1, &
                          epsilondev_xx_loc_m1,epsilondev_yy_loc_m1,epsilondev_xy_loc_m1,& 
                          epsilondev_xz_loc_m1,epsilondev_yz_loc_m1)
  ...
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: kappastore,mustore
  real(kind=CUSTOM_REAL), dimension(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB) :: factor_common,factor_common_kappa
  real(kind=CUSTOM_REAL), dimension(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB) :: R_trace_m1,R_xx_m1,R_yy_m1,& 
                                                                                     R_xy_m1,R_xz_m1,R_yz_m1

  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY) :: &
            epsilondev_xx_m1,epsilondev_yy_m1,epsilondev_xy_m1,epsilondev_xz_m1,epsilondev_yz_m1
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY) :: epsilondev_trace_m1
...
! local parameters
  integer :: i,j,k
  real(kind=CUSTOM_REAL) :: Sn_m1,Snp1_m1
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: factor_loc_m1
  ...
  do k = 1,NGLLZ
    do j = 1,NGLLY
      do i = 1,NGLLX
        ...
        ! term in trace
        factor_loc_m1(:) = kappastore(i,j,k,ispec)* factor_common_kappa(:,i,j,k,ispec)

        Sn_m1   = epsilondev_trace_m1(i,j,k,ispec)
        Snp1_m1   = epsilondev_trace_loc_m1(i,j,k)
        R_trace_m1(:,i,j,k,ispec) = alphaval(:) * R_trace_m1(:,i,j,k,ispec) + factor_loc_m1(:) * (betaval(:) * & 
                                                   Sn_m1 + gammaval(:) * Snp1_m1)

        ! term in xx yy zz xy xz yz
        factor_loc_m1(:) = mustore(i,j,k,ispec) * factor_common(:,i,j,k,ispec)

        ! term in xx
        Sn_m1   = epsilondev_xx_m1(i,j,k,ispec)
        Snp1_m1   = epsilondev_xx_loc_m1(i,j,k)
        R_xx_m1(:,i,j,k,ispec) = alphaval(:) * R_xx_m1(:,i,j,k,ispec) + factor_loc_m1(:) * (betaval(:) * & 
                                                  Sn_m1 + gammaval(:) * Snp1_m1)

        ! term in yy
        Sn_m1   = epsilondev_yy_m1(i,j,k,ispec)
        Snp1_m1   = epsilondev_yy_loc_m1(i,j,k)
        R_yy_m1(:,i,j,k,ispec) = alphaval(:) * R_yy_m1(:,i,j,k,ispec) + factor_loc_m1(:) * (betaval(:) * & 
                                                  Sn_m1 + gammaval(:) * Snp1_m1)

        ! term in zz not computed since zero trace

        ! term in xy
        Sn_m1   = epsilondev_xy_m1(i,j,k,ispec)
        Snp1_m1   = epsilondev_xy_loc_m1(i,j,k)
        R_xy_m1(:,i,j,k,ispec) = alphaval(:) * R_xy_m1(:,i,j,k,ispec) + factor_loc_m1(:) * (betaval(:) * & 
                                                  Sn_m1 + gammaval(:) * Snp1_m1)

        ! term in xz
        Sn_m1   = epsilondev_xz_m1(i,j,k,ispec)
        Snp1_m1   = epsilondev_xz_loc_m1(i,j,k)
        R_xz_m1(:,i,j,k,ispec) = alphaval(:) * R_xz_m1(:,i,j,k,ispec) + factor_loc_m1(:) * (betaval(:) * &
                                                  Sn_m1 + gammaval(:) * Snp1_m1)

        ! term in yz
        Sn_m1   = epsilondev_yz_m1(i,j,k,ispec)
        Snp1_m1   = epsilondev_yz_loc_m1(i,j,k)
        R_yz_m1(:,i,j,k,ispec) = alphaval(:) * R_yz_m1(:,i,j,k,ispec) + factor_loc_m1(:) * (betaval(:) * & 
                                                  Sn_m1 + gammaval(:) * Snp1_m1)
...
end subroutine compute_element_att_memory_second_order_rk_m1
