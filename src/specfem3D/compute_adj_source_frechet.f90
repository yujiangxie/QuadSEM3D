...
!lucas, out: eps_s(NDIM,NDIM), eps_m_s(NDIM), i.e., ! adjoint source frechet derivatives
!lucas, in: ispec,displ_s,Mxx,Myy,Mzz,Mxy,Mxz,Myz
...
! dudx  !1.1 lucas, to get duxdxl, ..., duzdzl
...
! strain eps_jk
        !1.2 lucas, to get eps_array(:,:,i,j,k) and eps_m_array(i,j,k) by f(eps, M)
        eps(1,1) = duxdxl
        ...
        eps_array(:,:,i,j,k) = eps(:,:)! lucas for eps_s

! Mjk eps_jk
        eps_m_array(i,j,k) = Mxx * eps(1,1) + Myy * eps(2,2) + Mzz * eps(3,3) + &
                   2 * (Mxy * eps(1,2) + Mxz * eps(1,3) + Myz * eps(2,3))!lucas for eps_m_s
  ...
  do k = 1,NGLLZ
    do j = 1,NGLLY
      do i = 1,NGLLX
        ...
        !2.2 lucas to get xix_s, etay_s, and gammaz_s, etc.
        if (ispec_irreg /= 0 ) then !irregular element
