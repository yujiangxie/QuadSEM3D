...
! writes out seismograms in SU (Seismic Unix) format
  ...
  ! local parameters
  character(len=MAX_STRING_LEN) :: procname,final_LOCAL_PATH,station_network !lucas
...
!  lucas changed to actual station_name and network_name, instead of the CPU
 do irec_local = 1,nrec_local
   irec = number_receiver_global(irec_local)
   write(station_network,"(a4,'.',a2)") station_name(irec),network_name(irec)
   print *, 'procname = ', trim(procname),' station_network = ',trim(station_network)
 enddo
  ...
  case (1,2,3)
    ...
    ! open seismograms displacement, velocity, acceleration
    if (seismo_offset == 0) then
      open(unit=IIN_SU1, file=trim(final_LOCAL_PATH)//trim(procname)//'_'//comp(istore)//'x_SU_m2', &
           status='replace', access='stream', form='unformatted', action='write', iostat=ier)
      if (ier /= 0) stop 'error opening ***x_SU_m2 file'
      open(unit=IIN_SU2, file=trim(final_LOCAL_PATH)//trim(procname)//'_'//comp(istore)//'y_SU_m2', &
           status='replace', access='stream', form='unformatted', action='write', iostat=ier)
      if (ier /= 0) stop 'error opening ***y_SU_m2 file'
      open(unit=IIN_SU3, file=trim(final_LOCAL_PATH)//trim(procname)//'_'//comp(istore)//'z_SU_m2', &
           status='replace', access='stream', form='unformatted', action='write', iostat=ier)
      if (ier /= 0) stop 'error opening ***z_SU_m2 file'
    else
      open(unit=IIN_SU1, file=trim(final_LOCAL_PATH)//trim(procname)//'_'//comp(istore)//'x_SU_m2', &
           status='old', access='stream', form='unformatted', action='readwrite', iostat=ier)
      if (ier /= 0) stop 'error opening ***x_SU_m2 file'
