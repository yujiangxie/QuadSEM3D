...
!------------------------------------------------------------------m2 ----------------------------------------
  !CTD_SEM
  subroutine compute_add_sources_viscoelastic_m2()
  ...
  use specfem_par, only: station_name,network_name,adj_source_file,num_free_surface_faces,free_surface_ispec, &
                        ...
                        ispec_selected_source,sourcearrays,SIMULATION_TYPE,NSTEP,READ_ADJSRC_ASDF, &
                        nrec,islice_selected_rec,ispec_selected_rec,nadj_rec_local, &
                        NTSTEP_BETWEEN_READ_ADJSRC,NOISE_TOMOGRAPHY,hxir_store,hetar_store,hgammar_store,source_adjoint_m2, & !lucas, use adjsrc(m1) for CTD_SEM, use adjsrc(m2) for FD
                        INVERSE_FWI_FULL_PROBLEM

  use specfem_par_elastic, only: accel, accel_m2,ispec_is_elastic !CTD_SEM
...
!    print *,'----------------------------'
          ...
          do k=1,NGLLZ
            do j=1,NGLLY
              do i=1,NGLLX
                iglob = ibool(i,j,k,ispec)
                accel_m2(:,iglob) = accel_m2(:,iglob) + sourcearrays(isource,:,i,j,k)*stf_used !CTD_SEM, sourcearrays sames for m1 and m2
              enddo
            enddo
          enddo
      ...
      if (ibool_read_adj_arrays .and. .not. INVERSE_FWI_FULL_PROBLEM) then
        ...
        if (.not. (SU_FORMAT .or. READ_ADJSRC_ASDF)) then
          ...
          do irec_local = 1, nrec_local
            ...
            irec = number_receiver_global(irec_local)
            adj_source_file = trim(network_name(irec))//'.'//trim(station_name(irec))
            call compute_arrays_adjoint_source(adj_source_file,irec_local) ! lucas, to get source_adjoint() and source_adjoint_m2() !CTD_SEM
          enddo
        else if (READ_ADJSRC_ASDF) then
          do irec_local = 1, nrec_local
            ! reads in **net**.**sta**.**BH**.adj files
...
!! DK DK Aug 2018: added this because Lei Zhang may have detected a problem
                    ...
                    accel_m2(:,iglob) = accel_m2(:,iglob) + & !CTD_SEM
                            source_adjoint_m2(:,irec_local,NTSTEP_BETWEEN_READ_ADJSRC - mod(it-1,NTSTEP_BETWEEN_READ_ADJSRC)) * & !lucas,use adjsrc(m1)forCTD_SEM,use adjsrc(m2)for FD
                            hxir_store(irec_local,i)*hetar_store(irec_local,j)*hgammar_store(irec_local,k)
                  enddo
                enddo
              enddo
...
!------------------------------------------------------------------m1 ----------------------------------------
  !CTD_SEM
  subroutine compute_add_sources_viscoelastic_m1()
  ...
  use specfem_par, only: station_name,network_name,adj_source_file,num_free_surface_faces,free_surface_ispec, &
                        ...
                        ispec_selected_source,sourcearrays,SIMULATION_TYPE,NSTEP,READ_ADJSRC_ASDF, &
                        nrec,islice_selected_rec,ispec_selected_rec,nadj_rec_local, &
                        NTSTEP_BETWEEN_READ_ADJSRC,NOISE_TOMOGRAPHY,hxir_store,hetar_store,hgammar_store,source_adjoint_m2, & ! lucas, use adjsrc(m2)
                        INVERSE_FWI_FULL_PROBLEM

  use specfem_par_elastic, only: accel, accel_m1,ispec_is_elastic !CTD_SEM
        ...
        if (ispec_is_elastic(ispec)) then
          ...
          do k=1,NGLLZ
            do j=1,NGLLY
              do i=1,NGLLX
                iglob = ibool(i,j,k,ispec)
                accel_m1(:,iglob) = accel_m1(:,iglob) + sourcearrays(isource,:,i,j,k)*stf_used !CTD_SEM, sourcearrays sames for m1 and m2
              enddo
            enddo
          enddo
...
!! DK DK Aug 2018: added this because Lei Zhang may have detected a problem
                    ...
                    accel_m1(:,iglob) = accel_m1(:,iglob) + & !CTD_SEM
                            source_adjoint_m2(:,irec_local,NTSTEP_BETWEEN_READ_ADJSRC - mod(it-1,NTSTEP_BETWEEN_READ_ADJSRC)) * &
                            hxir_store(irec_local,i)*hetar_store(irec_local,j)*hgammar_store(irec_local,k) ! lucas, use adjsrc(m2)
                  enddo
                enddo
              enddo
            endif ! ispec_is_elastic
...
!----------------------------------------- m2 -----------------------------------------------
! CTD_SEM
  subroutine compute_add_sources_viscoelastic_backward_m2()
  ...
  use specfem_par_elastic, only: b_accel, b_accel_m2,ispec_is_elastic !CTD_SEM
...
! adjoint simulations
        ...
        if (ispec_is_elastic(ispec)) then
          ...
          do k=1,NGLLZ
            do j=1,NGLLY
              do i=1,NGLLX
                iglob = ibool(i,j,k,ispec)
                b_accel_m2(:,iglob) = b_accel_m2(:,iglob) + sourcearrays(isource,:,i,j,k) * stf_used ! CTD_SEM
              enddo
            enddo
          enddo
  ...
  end subroutine compute_add_sources_viscoelastic_backward_m2
