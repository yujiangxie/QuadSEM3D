        ...
        if (USE_FORCE_POINT_SOURCE) then
          ...
          if (ispec_is_elastic(ispec) .or. ispec_is_poroelastic(ispec)) then
            ...
            call compute_arrays_source_forcesolution(sourcearray,hxis,hetas,hgammas,factor_source, &
                                                     comp_x,comp_y,comp_z,nu_source(:,:,isource)) !lucas, one out: sourcearray by the other inputs
          endif
  ...
  if (SIMULATION_TYPE == 2 .or. SIMULATION_TYPE == 3) then
          ...
          if (.not. READ_ADJSRC_ASDF) then
            ...
            do icomp = 1,NDIM
              filename = OUTPUT_FILES(1:len_trim(OUTPUT_FILES))// &
                       'SEM/'//trim(adj_source_file) // '.'// comp(icomp) // '.adj' ! changed by lucas
    ...
    if (myrank == 0) then
      ...
      if (nadj_files_found_tot == 0) then
        ...
        print *,'in directory : ',OUTPUT_FILES(1:len_trim(OUTPUT_FILES))//'SEM/' ! changed by lucas
        if (.not. SU_FORMAT) then
    ...
    if(CTD_SEM) then !---------------------m2----------
    allocate(source_adjoint_m2(NDIM,nadj_rec_local,NTSTEP_BETWEEN_READ_ADJSRC),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 2073')
    if (ier /= 0) stop 'error allocating array source_adjoint'
    endif !--------------------------------m2-----------
...
!
  ...
  if (nrec_local > 0) then
    ...
    ! allocate seismogram array
    if (SAVE_SEISMOGRAMS_DISPLACEMENT) then
      allocate(seismograms_d_m2(NDIM,nrec_local,NTSTEP_BETWEEN_OUTPUT_SEISMOS),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2087 for m2')
    else
      allocate(seismograms_d_m2(1,1,1),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2088 for m2')
    endif
    if (ier /= 0) stop 'error allocating array seismograms_d_m2'

    if (SAVE_SEISMOGRAMS_VELOCITY) then
      allocate(seismograms_v_m2(NDIM,nrec_local,NTSTEP_BETWEEN_OUTPUT_SEISMOS),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2089 for m2')
    else
      allocate(seismograms_v_m2(1,1,1),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2090 for m2')
    endif
    if (ier /= 0) stop 'error allocating array seismograms_v_m2'

    if (SAVE_SEISMOGRAMS_ACCELERATION) then
      allocate(seismograms_a_m2(NDIM,nrec_local,NTSTEP_BETWEEN_OUTPUT_SEISMOS),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2091 for m2')
    else
      allocate(seismograms_a_m2(1,1,1),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2092 for m2')
    endif
    if (ier /= 0) stop 'error allocating array seismograms_a_m2'

    ! initialize seismograms
    seismograms_d_m2(:,:,:) = 0._CUSTOM_REAL
    seismograms_v_m2(:,:,:) = 0._CUSTOM_REAL
    seismograms_a_m2(:,:,:) = 0._CUSTOM_REAL

    endif !---------------------------------------------------------------------------m2

  endif !lucas, nrec_local > 0
