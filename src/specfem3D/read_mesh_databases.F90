...
! United States and French Government Sponsorship Acknowledged.
  ...
  use specfem_par !lucas includes shared_parameters
...
! info about external mesh simulation
  ...
  if(CTD_SEM) then !------
  call bcast_all_cr_for_database(kappastore_m2(1,1,1,1), size(kappastore_m2))
  call bcast_all_cr_for_database(mustore_m2(1,1,1,1), size(mustore_m2))  
  endif !-----------------
...
! thus we now allocate it and read it in all cases (whether the simulation is acoustic, elastic, or acoustic/elastic)
  ...
  if(CTD_SEM) then !------------------------
  allocate(rhostore_m2(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 1426 for m2')
  if (ier /= 0) stop 'Error allocating array rhostore_m2'
  if (I_should_read_the_database) read(27) rhostore_m2
  call bcast_all_cr_for_database(rhostore_m2(1,1,1,1), size(rhostore_m2))
  endif !-----------------------------------
  ...
  if (ELASTIC_SIMULATION) then
    ...
    if(CTD_SEM) then !---------------------------------------------------------------------------------------------m2
    ! displacement,velocity,acceleration
    allocate(displ_m2(NDIM,NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1427 for m2')
    if (ier /= 0) stop 'Error allocating array displ_m2'
    allocate(veloc_m2(NDIM,NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1428 for m2')
    if (ier /= 0) stop 'Error allocating array veloc_m2'
    allocate(accel_m2(NDIM,NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1429 for m2')
    if (ier /= 0) stop 'Error allocating array accel_m2'
    if (SIMULATION_TYPE /= 1) then
      allocate(accel_adj_coupling_m2(NDIM,NGLOB_AB),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1430 for m2')
      if (ier /= 0) stop 'Error allocating array accel_adj_coupling_m2'
    endif

    ! allocates mass matrix
    allocate(rmass_m2(NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1431 for m2')

    if (ier /= 0) stop 'Error allocating array rmass_m2'
    ! initializes mass matrix contributions
    allocate(rmassx_m2(NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1432 for m2')
    allocate(rmassy_m2(NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1433 for m2')
    allocate(rmassz_m2(NGLOB_AB), stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1434 for m2')
    if (ier /= 0) stop 'Error allocating array rmassx_m2,rmassy_m2,rmassz_m2'
    rmassx_m2(:) = 0._CUSTOM_REAL
    rmassy_m2(:) = 0._CUSTOM_REAL
    rmassz_m2(:) = 0._CUSTOM_REAL
    if(compute_approx_Hessian) then !-----------------------------------------------m1---
    ! displacement,velocity,acceleration
    allocate(displ_m1(NDIM,NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1427 for m1')
    if (ier /= 0) stop 'Error allocating array displ_m1'
    allocate(veloc_m1(NDIM,NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1428 for m1')
    if (ier /= 0) stop 'Error allocating array veloc_m1'
    allocate(accel_m1(NDIM,NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1429 for m1')
    if (ier /= 0) stop 'Error allocating array accel_m1'
    if (SIMULATION_TYPE /= 1) then
      allocate(accel_adj_coupling_m1(NDIM,NGLOB_AB),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1430 for m1')
      if (ier /= 0) stop 'Error allocating array accel_adj_coupling_m1'
    endif

    ! allocates mass matrix
    allocate(rmass_m1(NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1431 for m1')

    if (ier /= 0) stop 'Error allocating array rmass_m1'
    ! initializes mass matrix contributions
    allocate(rmassx_m1(NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1432 for m1')
    allocate(rmassy_m1(NGLOB_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1433 for m1')
    allocate(rmassz_m1(NGLOB_AB), stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1434 for m1')
    if (ier /= 0) stop 'Error allocating array rmassx_m1,rmassy_m1,rmassz_m1'
    rmassx_m1(:) = 0._CUSTOM_REAL
    rmassy_m1(:) = 0._CUSTOM_REAL
    rmassz_m1(:) = 0._CUSTOM_REAL
    endif !----------------------------------------------------------------------------m1
    allocate(rho_vp_m2(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1435 for m2')
    if (ier /= 0) stop 'Error allocating array rho_vp_m2'
    allocate(rho_vs_m2(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1436 for m2')
    if (ier /= 0) stop 'Error allocating array rho_vs'
    endif !--------------------------------------------------------------------------------------------------------m2
    ...
    if(CTD_SEM) then !--------------------------------------------------------------------------------------------------m2, attenuation
    ! note: currently, they need to be defined, as they are used in some subroutine arguments
    allocate(R_xx_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1458 for m2')
    allocate(R_yy_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1459 for m2')
    allocate(R_xy_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1460 for m2')
    allocate(R_xz_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1461 for m2')
    allocate(R_yz_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1462 for m2')
    if (ier /= 0) stop 'Error allocating array R_xx_m2 etc.'

    ! needed for attenuation and/or kernel computations
    allocate(epsilondev_xx_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1463 for m2')
    allocate(epsilondev_yy_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1464 for m2')
    allocate(epsilondev_xy_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1465 for m2')
    allocate(epsilondev_xz_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1466 for m2')
    allocate(epsilondev_yz_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1467 for m2')
    allocate(epsilondev_trace_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1468 for m2')
    if (ier /= 0) stop 'Error allocating array epsilondev_xx_m2 etc.'

    allocate(R_trace_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1469 for m2')
    if (ier /= 0) stop 'Error allocating array R_trace_m2 etc.'

    ! note: needed for some subroutine arguments
    allocate(epsilon_trace_over_3_m2(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1470 for m2')
    if (ier /= 0) stop 'Error allocating array epsilon_trace_over_3_m2'
    if(compute_approx_Hessian) then !------------------------------------------------------------m1
    ! note: currently, they need to be defined, as they are used in some subroutine arguments
    allocate(R_xx_m1(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1458 for m1')
    allocate(R_yy_m1(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1459 for m1')
    allocate(R_xy_m1(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1460 for m1')
    allocate(R_xz_m1(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1461 for m1')
    allocate(R_yz_m1(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1462 for m1')
    if (ier /= 0) stop 'Error allocating array R_xx_m1 etc.'

    ! needed for attenuation and/or kernel computations
    allocate(epsilondev_xx_m1(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1463 for m1')
    allocate(epsilondev_yy_m1(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1464 for m1')
    allocate(epsilondev_xy_m1(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1465 for m1')
    allocate(epsilondev_xz_m1(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1466 for m1')
    allocate(epsilondev_yz_m1(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1467 for m1')
    allocate(epsilondev_trace_m1(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1468 for m1')
    if (ier /= 0) stop 'Error allocating array epsilondev_xx_m1 etc.'

    allocate(R_trace_m1(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1469 for m1')
    if (ier /= 0) stop 'Error allocating array R_trace_m1 etc.'

    ! note: needed for some subroutine arguments
    allocate(epsilon_trace_over_3_m1(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1470 for m1')
    if (ier /= 0) stop 'Error allocating array epsilon_trace_over_3_m1'
    endif !-------------------------------------------------------------------------------------m1
    ! needed for attenuation
    allocate(factor_common_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1471 for m2')
    if (ier /= 0) stop 'Error allocating array factor_common_m2 etc.'

    allocate(factor_common_kappa_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1472 for m2')
    if (ier /= 0) stop 'Error allocating array factor_common_kappa_m2 etc.'
    endif !---------------------------------------------------------------------------------------------------------------------m2, attenuation
    ...
    if(CTD_SEM) then !-------------------------------m2
    ! reads mass matrices
    if (I_should_read_the_database) read(27,iostat=ier) rmass_m2
    call bcast_all_cr_for_database(rmass_m2(1), size(rmass_m2))
    if (ier /= 0) stop 'Error reading in array rmass_m2'
    endif !------------------------------------------m2
    ...
    if(CTD_SEM) then !-------------------------------
    !pll material parameters for stacey conditions
    if (I_should_read_the_database) read(27,iostat=ier) rho_vp_m2
    if (size(rho_vp_m2) > 0) call bcast_all_cr_for_database(rho_vp_m2(1,1,1,1), size(rho_vp_m2))
    if (ier /= 0) stop 'Error reading in array rho_vp_m2'
    if (I_should_read_the_database) read(27,iostat=ier) rho_vs_m2
    if (size(rho_vs_m2) > 0) call bcast_all_cr_for_database(rho_vs_m2(1,1,1,1), size(rho_vs_m2))
    if (ier /= 0) stop 'Error reading in array rho_vs_m2'
    endif !------------------------------------------
  else
  ...
  if (num_abs_boundary_faces > 0) then
    ...
    if (STACEY_ABSORBING_CONDITIONS .and. (.not. PML_CONDITIONS)) then
      ...
      if (ELASTIC_SIMULATION) then
        ...
        if (size(rmassx_m2) > 0) call bcast_all_cr_for_database(rmassx_m2(1), size(rmassx_m2))
        if (size(rmassy_m2) > 0) call bcast_all_cr_for_database(rmassy_m2(1), size(rmassy_m2))
        if (size(rmassz_m2) > 0) call bcast_all_cr_for_database(rmassz_m2(1), size(rmassz_m2))
        endif !---------------------------m2
...
!      print *, 'in read_mesh_databases'
!      print *,'-------m1------------'
!      print *,'rmass(100)',rmass(100)
!      print *,'rmassx(100)',rmassx(100)
!      print *,'rmassy(100)',rmassy(100)
!      print *,'rmassz(100)',rmassz(100)

!      print *,'-------m2------------'
!      print *,'rmass_m2(100)',rmass_m2(100)
!      print *,'rmassx_m2(100)',rmassx_m2(100)
!      print *,'rmassy_m2(100)',rmassy_m2(100)
!      print *,'rmassz_m2(100)',rmassz_m2(100)
...
!
  ...
  if (ELASTIC_SIMULATION .and. SAVE_MOHO_MESH .and. SIMULATION_TYPE == 3) then
    ...
    ! dummy
    NSPEC2D_MOHO = 1
  endif ! lucas,! checks if anything to do
...
! reads in Moho meshes
  ...
  if (ELASTIC_SIMULATION .and. SIMULATION_TYPE == 3) then
    ...
    if(CTD_SEM) then !------------m2
    allocate(rho_kl_m2(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1590 for m2')
    if (ier /= 0) stop 'Error allocating array rho_kl_m2'
    endif !-----------------------m2
    ...
    if (ANISOTROPIC_KL) then
      ...
      if(CTD_SEM) then !---------------------m2
      allocate(mu_kl_m2(1,1,1,1),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1592 for m2')
      allocate(kappa_kl_m2(1,1,1,1),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1593 for m2')
      endif !--------------------------------m2
      ...
      if(CTD_SEM) then !------------------------------------m2
      ! shear modulus kernel
      allocate(mu_kl_m2(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1594 for m2')
      if (ier /= 0) stop 'Error allocating array mu_kl_m2'
      ! compressional modulus kernel
      allocate(kappa_kl_m2(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1595 for m2')
      if (ier /= 0) stop 'Error allocating array kappa_kl_m2'
      endif !-----------------------------------------------m2
    ...
    if (CTD_SEM) then !-----------------------------------------------m2--------------------------------------
    ! backward displacement,velocity,acceleration fields
    allocate(b_displ_m2(NDIM,NGLOB_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1587 for m2')
    if (ier /= 0) stop 'Error allocating array b_displ_m2'
    allocate(b_veloc_m2(NDIM,NGLOB_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1588 for m2')
    if (ier /= 0) stop 'Error allocating array b_veloc_m2'
    allocate(b_accel_m2(NDIM,NGLOB_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1589 for m2')
    if (ier /= 0) stop 'Error allocating array b_accel_m2'

    ! adjoint kernels

    ! primary, isotropic kernels
    ! density kernel
    allocate(rho_kl_Ha(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1590 for Ha')
    if (ier /= 0) stop 'Error allocating array rho_kl_Ha'
    allocate(rho_kl_Hbm(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1590 for Hbm')
    if (ier /= 0) stop 'Error allocating array rho_kl_Hbm'
    if(compute_approx_Hessian) then !---------------------------------------------
     allocate(rho_kl_Hbs(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
     if (ier /= 0) call exit_MPI_without_rank('error allocating array 1590 for Hbs')
     if (ier /= 0) stop 'Error allocating array rho_kl_Hbs'
    endif !------------------------------------------------------------------------
    allocate(rho_kl_Hc(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1590 for Hc')
    if (ier /= 0) stop 'Error allocating array rho_kl_Hc'
    allocate(rho_kl_Habc(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1590 for Habc')
    if (ier /= 0) stop 'Error allocating array rho_kl_Habc'
    allocate(rho_kl_Hdiff(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1590 for Hdiff')
    if (ier /= 0) stop 'Error allocating array rho_kl_Hdiff'
...
!    else
      ! shear modulus kernel
      allocate(mu_kl_Ha(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1594 for Ha')
      if (ier /= 0) stop 'Error allocating array mu_kl_Ha'
      allocate(mu_kl_Hbm(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1594 for Hbm')
      if (ier /= 0) stop 'Error allocating array mu_kl_Hbm'
      if(compute_approx_Hessian) then !---------------------------------------------
       allocate(mu_kl_Hbs(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
       if (ier /= 0) call exit_MPI_without_rank('error allocating array 1594 for Hbs')
       if (ier /= 0) stop 'Error allocating array mu_kl_Hbs'
      endif !-----------------------------------------------------------------------
      allocate(mu_kl_Hc(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1594 for Hc')
      if (ier /= 0) stop 'Error allocating array mu_kl_Hc'
      allocate(mu_kl_Habc(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1594 for Habc')
      if (ier /= 0) stop 'Error allocating array mu_kl_Habc'
      allocate(mu_kl_Hdiff(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1594 for Hdiff')
      if (ier /= 0) stop 'Error allocating array mu_kl_Hdiff'
      ! compressional modulus kernel
      allocate(kappa_kl_Ha(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1595 for Ha')
      if (ier /= 0) stop 'Error allocating array kappa_kl_Ha'
      allocate(kappa_kl_Hbm(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1595 for Hbm')
      if (ier /= 0) stop 'Error allocating array kappa_kl_Hbm'
      if(compute_approx_Hessian) then !----------------------------------------------
       allocate(kappa_kl_Hbs(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
       if (ier /= 0) call exit_MPI_without_rank('error allocating array 1595 for Hbs')
       if (ier /= 0) stop 'Error allocating array kappa_kl_Hbs'
      endif !------------------------------------------------------------------------
      allocate(kappa_kl_Hc(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1595 for Hc')
      if (ier /= 0) stop 'Error allocating array kappa_kl_Hc'
      allocate(kappa_kl_Habc(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1595 for Habc')
      if (ier /= 0) stop 'Error allocating array kappa_kl_Habc'
      allocate(kappa_kl_Hdiff(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1595 for Hdiff')
      if (ier /= 0) stop 'Error allocating array kappa_kl_Hdiff'
      !dummy
!      allocate(cijkl_kl(1,1,1,1,1),stat=ier)
!      if (ier /= 0) call exit_MPI_without_rank('error allocating array 1596')
!    endif
...
!    if (ier /= 0) stop 'Error allocating array b_request_send_vector_ext_mesh etc.'
    ...
    ! allocates attenuation solids
    allocate(b_R_xx_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1610 for m2')
    allocate(b_R_yy_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1611 for m2')
    allocate(b_R_xy_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1612 for m2')
    allocate(b_R_xz_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1613 for m2')
    allocate(b_R_yz_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1614 for m2')
    if (ier /= 0) stop 'Error allocating array b_R_xx_m2 etc.'

    ! note: these arrays are needed for attenuation and/or kernel computations
    allocate(b_epsilondev_xx_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1615 for m2')
    allocate(b_epsilondev_yy_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1616 for m2')
    allocate(b_epsilondev_xy_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1617 for m2')
    allocate(b_epsilondev_xz_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1618 for m2')
    allocate(b_epsilondev_yz_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1619 for m2')
    allocate(b_epsilondev_trace_m2(NGLLX,NGLLY,NGLLZ,NSPEC_STRAIN_ONLY),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1620 for m2')
    if (ier /= 0) stop 'Error allocating array b_epsilondev_xx_m2 etc.'
    ! needed for kernel computations
    allocate(b_epsilon_trace_over_3_m2(NGLLX,NGLLY,NGLLZ,NSPEC_ADJOINT),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1621_m2')
    if (ier /= 0) stop 'Error allocating array b_epsilon_trace_over_3_m2'

    ! allocates attenuation solids for considering kappa
    allocate(b_R_trace_m2(N_SLS,NGLLX,NGLLY,NGLLZ,NSPEC_ATTENUATION_AB),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1622 for m2')
    if (ier /= 0) stop 'Error allocating array b_R_trace_m2 etc.'
...
!    endif
    ...
    if(CTD_SEM) then !----------------------------------------------------m2
    ! dummy allocation
    allocate(b_displ_m2(1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1624 for m2')
    if (ier /= 0) stop 'Error allocating dummy array b_displ_m2'
    allocate(b_veloc_m2(1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1625 for m2')
    if (ier /= 0) stop 'Error allocating dummy array b_veloc_m2'
    allocate(b_accel_m2(1,1),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 1626 for m2')
    if (ier /= 0) stop 'Error allocating dummy array b_accel_m2'
    endif !---------------------------------------------------------------m2
