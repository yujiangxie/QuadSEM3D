...
! writes the seismograms with time shift
  ...
  if (SIMULATION_TYPE == 3 .and. (.not. SAVE_SEISMOGRAMS_IN_ADJOINT_RUN)) then
    ...
    do_save_seismograms = .true. !lucas SIMULATION_TYPE == 1
  endif
  ...
  if (GPU_MODE) then
    ...
    ! on CPU
    if (nrec_local > 0 .and. do_save_seismograms) then
      call compute_seismograms() ! CTD_SEM
    endif
  endif
      ...
      if (nrec_local > 0 .or. (WRITE_SEISMOGRAMS_BY_MASTER .and. myrank == 0) .or. ASDF_FORMAT) then
        ...
        case (1,3)
          if (.not. SU_FORMAT) then
            ...
            if(CTD_SEM) then !------------------------------------m2, have done for m2, used for my test
            if (SAVE_SEISMOGRAMS_DISPLACEMENT) &
              call write_seismograms_to_file_m2(seismograms_d_m2,1)
            if (SAVE_SEISMOGRAMS_VELOCITY) &
              call write_seismograms_to_file_m2(seismograms_v_m2,2)
            if (SAVE_SEISMOGRAMS_ACCELERATION) &
              call write_seismograms_to_file_m2(seismograms_a_m2,3)
            endif !-----------------------------------------------m2
          else
            ! SU_format
            ...
            if(CTD_SEM) then !----------------------------------m2, need to do for m2
            if (SAVE_SEISMOGRAMS_DISPLACEMENT) &
              call write_output_SU(seismograms_d_m2,1)
            if (SAVE_SEISMOGRAMS_VELOCITY) &
              call write_output_SU(seismograms_v_m2,2)
            if (SAVE_SEISMOGRAMS_ACCELERATION) &
              call write_output_SU(seismograms_a_m2,3)
            endif !---------------------------------------------m2
          endif
        case (2)
          ! adjoint wavefield
          if (.not. SU_FORMAT) then
            ! adjoint simulations
            if (SAVE_SEISMOGRAMS_DISPLACEMENT) &
              call write_adj_seismograms_to_file(seismograms_d,1)! lucas, wirte t and seismograms_d of each line of the file
            if (SAVE_SEISMOGRAMS_VELOCITY) &
            ...
            if(CTD_SEM) then !------------------------------------m2, need to do for m2
            if (SAVE_SEISMOGRAMS_DISPLACEMENT) &
              call write_adj_seismograms_to_file(seismograms_d_m2,1)! 
            if (SAVE_SEISMOGRAMS_VELOCITY) &
              call write_adj_seismograms_to_file(seismograms_v_m2,2)
            if (SAVE_SEISMOGRAMS_ACCELERATION) &
              call write_adj_seismograms_to_file(seismograms_a_m2,3)
            endif !-----------------------------------------------m2
          else
            ! SU_format
            ! write ONE binary file for all receivers (nrec_local) within one proc
...
! write seismograms to text files
  ...
  subroutine write_seismograms_to_file(seismograms,istore)! lucas, istore = 1, 2, 3, 4 corresponds to d, v, a, p.
  ...
  !1.lucas, to save file into 'all_seismograms_component_node_myrank'
  if (.not. WRITE_SEISMOGRAMS_BY_MASTER) then
    ...
    endif ! lucas ,SAVE_ALL_SEISMOS_IN_ONE_FILE

    !1.2 lucas, initialize asdf_container%receriver_name_array(nrec_local), ...., asdf_container%records(3*nrec_local)
  ...
  !3.lucas, no processor or myrank involved.
...
!---------------------------------------------m2-----------------------------------------------------------
  ...
  subroutine write_seismograms_to_file_m2(seismograms,istore)! lucas, istore = 1, 2, 3, 4 corresponds to d, v, a, p.
...
!      endif
      ...
      if (USE_BINARY_FOR_SEISMOGRAMS) then
        if (seismo_offset == 0) then
          open(unit=IOUT,file=trim(OUTPUT_FILES)//trim(sisname)//'.bin_m2',status='unknown', &
               form='unformatted',action='write')
        else
          open(unit=IOUT,file=trim(OUTPUT_FILES)//trim(sisname)//'.bin_m2',status='old',position='append', &
               form='unformatted',action='write')
        endif
      else
        if (seismo_offset == 0) then
          open(unit=IOUT,file=trim(OUTPUT_FILES)//trim(sisname)//'.ascii_m2',status='unknown', &
               form='formatted',action='write')
        else
          open(unit=IOUT,file=trim(OUTPUT_FILES)//trim(sisname)//'.ascii_m2',status='old',position='append', &
               form='formatted',action='write')
        endif
      endif
      endif !--------------------------------------------------m2
...
!      call write_one_seismogram(one_seismogram,irec_local,irec,component,istore)! CTD_SEM, this works for e.g., XS.S01D.BXE.semd 
      if(CTD_SEM) then !-----------------m2
      call write_one_seismogram_m2(one_seismogram,irec_local,irec,component,istore)! CTD_SEM, this works for e.g., XS.S01D.BXE.semd_m2
      endif !----------------------------m2
    enddo ! nrec_local
  ...
  !2,lucas 
    ...
    do irec_local = 1, nrec_local
      ...
      irec = number_receiver_global(irec_local)
      one_seismogram(:,:) = seismograms(:,irec_local,:)
      call write_one_seismogram_m2(one_seismogram,irec_local,irec,component,istore) ! CTD_SEM
    enddo
    call write_asdf()
    call synchronize_all()
    call close_asdf_data()

  !3.lucas, CTD_SEM, no processor or myrank involved.
! only the master process does the writing of seismograms and
! collects the data from all other processes
  else
    ! only written out by master process - WRITE_SEISMOGRAMS_BY_MASTER
...
!-----------------------------------------------m2 --------------------------------------------------
  subroutine write_one_seismogram_m2(one_seismogram,irec_local,irec,component,istore)
  ...
  integer :: iorientation,number_of_components
  integer :: length_station_name,length_network_name
  character(len=MAX_STRING_LEN) :: sisname_m2,final_LOCAL_PATH ! CTD_SEM
  character(len=3) :: channel
...
!                                                station_name(irec)(1:length_station_name),channel,component
    ...
    write(sisname_m2,"(a,'.',a,'.',a3,'.sem',a1,'_m2')") network_name(irec)(1:length_network_name), &                   
                                                station_name(irec)(1:length_station_name),channel,component
...
!                                      iorientation,sisname,final_LOCAL_PATH)
    ...
    call write_output_ASCII_or_binary(one_seismogram, &
                                      NSTEP,it,SIMULATION_TYPE,DT,t0, &
                                      iorientation,sisname_m2,final_LOCAL_PATH) ! CTD_SEM

    endif

  enddo ! do iorientation

  end subroutine write_one_seismogram_m2
