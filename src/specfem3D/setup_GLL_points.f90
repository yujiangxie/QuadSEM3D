...
! United States and French Government Sponsorship Acknowledged.
  ...
  if (ELASTIC_SIMULATION) then
    ...
    if(CTD_SEM) then !-----------------------------------m2, lucas, min_resolved_period_m2 is computed by input velocity vp
    call check_mesh_resolution(myrank,NSPEC_AB,NGLOB_AB, &  
                               ibool,xstore,ystore,zstore, &
                               kappastore_m2,mustore_m2,rho_vp_m2,rho_vs_m2, &
                               DT,model_speed_max_m2,min_resolved_period_m2, &
                               LOCAL_PATH,SAVE_MESH_FILES)
    endif !----------------------------------------------m2
    ...
    call check_mesh_resolution(myrank,NSPEC_AB,NGLOB_AB, &
                               ...
                               kappastore,mustore,rho_vp,rho_vs, &
                               DT,model_speed_max,min_resolved_period, &
                               LOCAL_PATH,SAVE_MESH_FILES) ! lucas return 'model_speed_max' and 'minimum_period_resolved'
    deallocate(rho_vp,rho_vs)
  endif
  ...
  allocate(hxir(NGLLX),stat=ier)! lucas, h
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 2425')
  allocate(hpxir(NGLLX),stat=ier)! lucas, hp
