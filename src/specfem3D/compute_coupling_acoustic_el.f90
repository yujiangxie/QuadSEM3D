...
! for acoustic solver
  !lucas, to update(inout) potential_dot_dot_acoustic
  subroutine compute_coupling_acoustic_el(NSPEC_AB,NGLOB_AB, &
                        ibool,displ,potential_dot_dot_acoustic, & !lucas, inout: potential_dot_dot_acoustic
        ...
        if (is_CPML(ispec)) then
          if (SIMULATION_TYPE == 1) then
            ...
            call pml_compute_memory_variables_acoustic_elastic(ispec_CPML,iface,iglob,i,j,k, &
                                                  displ_x,displ_y,displ_z,displ, & !lucas, here displ_x,displ_y,displ_z,are completely obtained in this subroutine, nothing to do the displ_x,displ_y,displ_z above.
                                                  num_coupling_ac_el_faces,rmemory_coupling_ac_el_displ)!lucas, out: displ_x,displ_y,displ_z
                                                                                                        !lucas, inout: rmemory_coupling_ac_el_displ(3,NGLLX,NGLLY,NGLLZ,num_coupling_ac_el_faces,2),
                                                                                                        !lucas, where computing rmemory_coupling_ac_el_displ is to compute displ_x, displ_y, displ_z.
                                                                                                        !lucas, in: displ
          endif

          if (SIMULATION_TYPE == 3) then
            ! safety stop            stop 'TODO: Coupling acoustic-elastic for CPML in compute_coupling_acoustic_el() not implemented yet...'

          endif
        endif
      endif
    
      !lucas, the displ_x,displ_y,displ_z either obtained in the adjoint wavefield case or the CPML overwrite cases. if the case of SIMULATION_TYPE /= 1, just change the sign.
