...
!           if (height >= 3000.d0) then
  ...
  do it = it_begin,it_end ! lucas-----------------------------------------------------------------------begin it --------------------------------------------
    !lucas 2.1
    ! simulation status output and stability check
    if (mod(it,NTSTEP_BETWEEN_OUTPUT_INFO) == 0 .or. it == it_begin + 4 .or. it == it_end) then
      call check_stability() ! lucas, CTD_SEM have done for m2 of the elastic version
    endif    
    ! simulation status output and stability check
    ...
    if (.not. USE_LDDRK) call update_displacement_scheme()! lucas, update displ and veloc, and set accel=0; have done for m2
    ...
    if (.not. GPU_MODE) then
      ...
      if (SIMULATION_TYPE == 3) then
        ...
        if (ELASTIC_SIMULATION .and. ACOUSTIC_SIMULATION) then
          ...
          if (ELASTIC_SIMULATION .and. CTD_SEM) then !--------------------------------------m2
             call compute_forces_viscoelastic_calling_m2()
             if(compute_approx_Hessian) then !--------------------m1, for approx Hessian
             call compute_forces_viscoelastic_calling_m1()
             endif !----------------------------------------------m1
          endif !---------------------------------------------------------------------------m2
        ...
        if (ELASTIC_SIMULATION .and. CTD_SEM) then !--------------------------------------m2
           call compute_forces_viscoelastic_backward_calling_m2()
        endif !---------------------------------------------------------------------------m2

      else !lucas, simulation_type !=3
        ! forward simulations
    ...
    if (SIMULATION_TYPE == 3 .and. it == 1) then
      call it_read_forward_arrays() ! CTD_SEM, have done for m1 and m2 for reading save_forward_arrays(_m2).bin
    endif
    ...
    ! lucas test output---------------------------------------------------------
!    ispec = ispec_selected_source(1)
!    iglob = ibool(2,2,2,ispec)
!    print *,'it=',it
!    print *,'------------m1----------------'
!    print *,'displ(3,iglob)', displ(3,iglob)
!    print *,'veloc(3,iglob)', veloc(3,iglob)
!    print *,'accel(3,iglob)', accel(3,iglob)
!    print *,'------------m2----------------'
!    print *,'displ_m2(3,iglob)', displ_m2(3,iglob)
!    print *,'veloc_m2(3,iglob)', veloc_m2(3,iglob)
!    print *,'accel_m2(3,iglob)', accel_m2(3,iglob)
!    print *,'----------------------------'
    ...
    call write_seismograms() ! lucas, have done for m1 and m2

    ! lucas 2.6
    ! adjoint simulations: kernels
    if (SIMULATION_TYPE == 3) then
      call compute_kernels() ! lucas, have done for m1 and m2
    endif

    ! lucas 2.7
    ! outputs movie files
    call write_movie_output() ! lucas, not do for m2, only the old verison is used for the orignal model for wavefield visualization
    ...
    if (VTK_MODE) then
      ! updates VTK window
      call vtk_window_update() ! lucas, not do for m2
    endif
#endif
  ...
  !
  enddo   ! end of main time loop, lucas--------------------------------------------------------end it----------------------------------------------------------------
...
!----  close energy file
  ...
  if (VTK_MODE) then
    ! frees memory
    call vtk_window_cleanup(do_restart)! lucas, not do for m2
...
!          that is, index it for adjoint sources will match index NSTEP - 1 for backward/reconstructed wavefields
  if (ADIOS_FOR_FORWARD_ARRAYS) then
    ...
    if(CTD_SEM) then !------------------m2
    open(unit=IIN+1500,file=trim(prname)//'save_forward_arrays_m2.bin',status='old', &
          action='read',form='unformatted',iostat=ier)
    if (ier /= 0) then
      print *,'error: opening save_forward_arrays_m2'
      print *,'path: ',trim(prname)//'save_forward_arrays_m2.bin'
      call exit_mpi(myrank,'error open file save_forward_arrays_m2.bin')
    endif
    endif !-----------------------------m2
    ...
    if (ELASTIC_SIMULATION) then
      ...
      if(CTD_SEM) then !---------------------------m2
      read(IIN+1500) b_displ_m2
      read(IIN+1500) b_veloc_m2
      read(IIN+1500) b_accel_m2
      ! memory variables if attenuation
      if (ATTENUATION) then
        read(IIN+1500) b_R_trace_m2
        read(IIN+1500) b_R_xx_m2
        read(IIN+1500) b_R_yy_m2
        read(IIN+1500) b_R_xy_m2
        read(IIN+1500) b_R_xz_m2
        read(IIN+1500) b_R_yz_m2
        read(IIN+1500) b_epsilondev_trace_m2
        read(IIN+1500) b_epsilondev_xx_m2
        read(IIN+1500) b_epsilondev_yy_m2
        read(IIN+1500) b_epsilondev_xy_m2
        read(IIN+1500) b_epsilondev_xz_m2
        read(IIN+1500) b_epsilondev_yz_m2
      endif
      endif !--------------------------------------m2

    endif !lucas, ELASTIC_SIMULATION
