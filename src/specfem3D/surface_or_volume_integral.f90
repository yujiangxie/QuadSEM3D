...
!=====================================================================
  ...
  !2.lucas 
  if ( (Surf_or_vol_integral == 2) .or. (Surf_or_vol_integral == 3) ) then ! lucas, 1 = Surface integral, 2 = Volume, 3 = Both
      ...
      do k = 1,NGLLZ
        do j = 1,NGLLY
          do i = 1,NGLLX
            ...
            !2.2 lucas, to get integr_volloc(:) 
...
!
  ...
  if ( (Surf_or_vol_integral == 1) .or. (Surf_or_vol_integral == 3) ) then
    ...
    !3.1 lucas, integr_bounloc(:), free_surface
...
!!          integr_bounloc(:) = integr_bounloc(:) + (free_surface_jacobian2Dw(igll,iface) * f_integrand_bounloc(:,iglob))
    ...
    !3.2 lucas, integr_bounloc(:), abs_boundary
...
!!          integr_bounloc(:) = integr_bounloc(:) + (abs_boundary_jacobian2Dw(igll,iface) * f_integrand_bounloc(:,iglob))
    ...
    !3.3 lucas, sum
    call sum_all_1Darray_dp(integr_bounloc, integral_boun, 3)!lucas,sum_all_1Darray_dp(sendbuf, recvbuf, nx), where nx=number of elements in send buffer,!!!Reduces values on all processes to a single value 
