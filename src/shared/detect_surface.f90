...
! adds contributions from different partitions to valence
  call assemble_MPI_scalar_i_blocking(NPROC,nglob,valence, &
                        num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                        nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                        my_neighbors_ext_mesh) ! lucas, valence=array_val in subrountine, could be 1,2,3,4 when 4 cpus used.
...
!
  ...
  subroutine ds_set_cross_section_flags(nspec,ispec_is_surface, &
                                        nglob,iglob_is_surface, &
                                        i,j,k,ispec,ibool, &
                                        valence,countval) ! lucas, ds=detect surface
  ...
  ! zmin & zmax face
  if ((k == 1 .or. k == NGLLZ) .and. valence(ibool(3,3,k,ispec)) >= 1) then ! lucas, the ">=1" here means this GLL point being recored. valence(:) = 0 if the GLL point is unface
    has_face = .true.
    do jj = 1, NGLLY
      do ii = 1, NGLLX
        iglob_is_surface(ibool(ii,jj,k,ispec)) = .true.
...
!
  ...
  call get_element_face_gll_indices(iface,face_ijk,NGLLX,NGLLX)! lucas, use ijk_face in subroutine which locaded in get_element_face.f90 of the the shared directory.
...
! determines spectral elements containing points on surfacevalence
  countval = 0
  do ispec = 1, nspec
    ! loops over GLL points not on edges or corners, but inside faces
    do k = 1, NGLLZ
