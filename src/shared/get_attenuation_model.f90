...
!
  ...
  subroutine get_attenuation_model(myrank,nspec,USE_OLSEN_ATTENUATION,OLSEN_ATTENUATION_RATIO, &
                                   ...
                                   ispec_is_elastic,min_resolved_period,prname,ATTENUATION_f0_REFERENCE) ! lucas. common function for m1 and m2

! precalculates attenuation arrays and stores arrays into files

  use constants

  use shared_parameters, only: MIN_ATTENUATION_PERIOD,MAX_ATTENUATION_PERIOD,COMPUTE_FREQ_BAND_AUTOMATIC, CTD_SEM !lucas
  ...
  if (myrank == 0) then
    ...
    call flush_IMAIN()
    if(CTD_SEM) then !-----
    write(IMAIN,*) "  Q_mu_m2 min/max           : ",sngl(qmin_all),sngl(qmax_all)
    write(IMAIN,*) "  Q_kappa_m2 min/max        : ",sngl(qmin_kappa_all),sngl(qmax_kappa_all)
    write(IMAIN,*)
    call flush_IMAIN()
    endif !----------------
  endif
  ...
  if(CTD_SEM) then !------------------------------------------------------------------- need to be optimal here using if, else, endif
  ! stores attenuation arrays into files
  open(unit=27+1, file=prname(1:len_trim(prname))//'attenuation_m2.bin', &
        status='unknown',action='write',form='unformatted',iostat=ier)
  if (ier /= 0) then
...
!-------------------------------------------------------------------------------------------------
!
  !lucas, input min_resolved_period to get MIN_ATTENUATION_PERIOD,MAX_ATTENUATION_PERIOD
  !lucase tau_sigma and f_c_source are computed by MIN_ATTENUATION_PERIOD,MAX_ATTENUATION_PERIOD
...
! returns: period band constants tau_sigma and center frequency f_c_source
  ...
  if (COMPUTE_FREQ_BAND_AUTOMATIC) then ! otherwise they were entered as input values by the user in the Par_file
    ...
    !lucas, use min_period to compute MIN_ATTENUATION_PERIOD and MAX_ATTENUATION_PERIOD
  ...
  ! lucas, tau_sigma is computed by N_SLS, MIN_ATTENUATION_PERIOD, and MAX_ATTENUATION_PERIOD
