...
! works for external, irregular meshes
  ...
  integer :: ispec, j, k ! lucas
  logical :: need_cut !lucas
  need_cut = .false.
...
!! DK DK remark: then why do you use MPI??? you could remove MPI entirely
  ...
  do it = 1, num_node
    ...
    !lucas times 2 for the depth for visualzition 
!    zstore=zstore*2;
    ...
    ! lucas added for cut the data_sp------------------
   if(need_cut) then 
    do ispec = 1, NSPEC_AB
     do k=1,NGLLZ
      do j=1,NGLLY
       do i=1,NGLLX
        if(data_sp(i,j,k,ispec)<= -5e-13) data_sp(i,j,k,ispec)=-5e-13
        if(data_sp(i,j,k,ispec)>=  5e-13) data_sp(i,j,k,ispec)= 5e-13
       enddo
      enddo
     enddo
