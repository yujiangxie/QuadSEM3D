...
! reads Databases files
  if (ADIOS_FOR_DATABASES) then
    ...
    call read_partition_files() ! lucas, got "nundefMat_ext_mesh", "undef_mat_prop(6,imat)", "nmat_ext_mesh", "mat_prop(17,imat)"
  endif

! external mesh creation
  call setup_mesh() ! Lucas, we use the above nundefMat_ext_mesh in read_model_tomography() of model_tomography_broadcast() of get_model(), 
