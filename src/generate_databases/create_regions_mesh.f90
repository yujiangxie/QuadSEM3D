...
! create the different regions of the mesh
  ...
  use generate_databases_par, only: nspec => NSPEC_AB,nglob => NGLOB_AB, &
      ...
      CTD_SEM, UNDO_ATTENUATION_AND_OR_PML !lucas
...
! memory size needed by the solver
  ...
  real(kind=CUSTOM_REAL) :: model_speed_max_m2,min_resolved_period_m2 !lucas, CTD_SEM
...
! checks the mesh, stability and resolved period
  ...
  if (POROELASTIC_SIMULATION) then
    ...
    call check_mesh_resolution(myrank,nspec,nglob_dummy, &
                               ...
                               -1.0d0,model_speed_max,min_resolved_period, &
                               LOCAL_PATH,SAVE_MESH_FILES) !lucas inout:model_speed_max,min_resolved_period, and others are in

    if(CTD_SEM) then !----------
    call check_mesh_resolution(myrank,nspec,nglob_dummy, &
                               ibool,xstore_dummy,ystore_dummy,zstore_dummy, &
                               kappastore_m2,mustore_m2,rho_vp_m2,rho_vs_m2, &
                               -1.0d0,model_speed_max_m2,min_resolved_period_m2, &
                               LOCAL_PATH,SAVE_MESH_FILES) !lucas inout:model_speed_max_m2,min_resolved_period_m2, and others are in
...
! saves binary mesh files for attenuation
  if (ATTENUATION) then
    ...
    call get_attenuation_model(myrank,nspec,USE_OLSEN_ATTENUATION,OLSEN_ATTENUATION_RATIO, & !lucas, to get attenuation.bin
    ...
    if(CTD_SEM .and. UNDO_ATTENUATION_AND_OR_PML) then !lucas, only used with attenuation ----------------------------
    !lucas, to get attenuation_m2.bin, use ATTENUATION or UNDO_ATTENUATION_AND_OR_PML could be same using pasimonious storage method
    call get_attenuation_model(myrank,nspec,USE_OLSEN_ATTENUATION,OLSEN_ATTENUATION_RATIO, &
                               mustore_m2,rho_vs_m2,kappastore_m2,rho_vp_m2,qkappa_attenuation_store_m2,qmu_attenuation_store_m2, &
                               ispec_is_elastic,min_resolved_period_m2,prname,ATTENUATION_f0_REFERENCE) !lucas, all are in
    endif !------------------------------------------------------------------------------------------------------
  endif
  ...
  if(CTD_SEM) then !---------m2
  deallocate(qkappa_attenuation_store_m2,qmu_attenuation_store_m2)
  deallocate(kappastore_m2,mustore_m2,rho_vp_m2,rho_vs_m2)
  endif !--------------------m2
  ...
  if (STACEY_ABSORBING_CONDITIONS) then
     if (ELASTIC_SIMULATION) then
       ...
       if(CTD_SEM) then !--------------m2
       deallocate(rmassx_m2,rmassy_m2,rmassz_m2)  
       endif !-------------------------m2
     endif
     if (ACOUSTIC_SIMULATION) then
...
subroutine crm_ext_allocate_arrays(nspec,LOCAL_PATH,myrank, &
  ...
  use generate_databases_par, only: STACEY_INSTEAD_OF_FREE_SURFACE,PML_INSTEAD_OF_FREE_SURFACE, &
    ...
    CTD_SEM !lucas add for allocate addtional variables for m2
...
! attenuation
  ...
  if(CTD_SEM) then !lucas --------------------
  allocate(qkappa_attenuation_store_m2(NGLLX,NGLLY,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 724 for m2')
  if (ier /= 0) call exit_MPI(myrank,'not enough memory to allocate arrays for m2')
  allocate(qmu_attenuation_store_m2(NGLLX,NGLLY,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 725 for m2')
  if (ier /= 0) call exit_MPI(myrank,'not enough memory to allocate arrays for m2')
  !------------------------------------------
  endif
...
! array with model density
  ...
  if(CTD_SEM) then !lucas ---------------
  allocate(rho_vp_m2(NGLLX,NGLLY,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 741 for m2')
  allocate(rho_vs_m2(NGLLX,NGLLY,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 742 for m2')
  if (ier /= 0) call exit_MPI(myrank,'not enough memory to allocate arrays for m2')
  ! array with model density
  allocate(rhostore_m2(NGLLX,NGLLY,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 743 for m2')
  allocate(kappastore_m2(NGLLX,NGLLY,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 744 for m2')
  allocate(mustore_m2(NGLLX,NGLLY,NGLLZ,nspec),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 745 for m2')
  if (ier /= 0) call exit_MPI(myrank,'not enough memory to allocate arrays for m2')
  endif !--------------------------------
