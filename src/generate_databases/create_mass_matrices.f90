...
! returns precomputed mass matrix in rmass array
  ...
  use generate_databases_par, only: CTD_SEM !lucas
  implicit none
...
! local parameters
  ...
  if (ELASTIC_SIMULATION) then
     ...
     if(CTD_SEM) then !------------
     ! allocates memory
     allocate(rmass_m2(nglob),stat=ier)
     if (ier /= 0) call exit_MPI_without_rank('error allocating array 660 for m2')
     if (ier /= 0) call exit_MPI_without_rank('error allocating array rmass_m2')
     rmass_m2(:) = 0._CUSTOM_REAL
     endif !------------------------

     ! returns elastic mass matrix
     if (PML_CONDITIONS) then
        call create_mass_matrices_pml_elastic(nspec,ibool)
     else ! lucas, not PML_CONDITIONS
        do ispec=1,nspec
              ...
              do k=1,NGLLZ
                 do j=1,NGLLY
                    do i=1,NGLLX
                       ...
                       if(CTD_SEM) then !----------------------------------m2         
                       rmass_m2(iglob) = rmass_m2(iglob) + &
                               real( dble(jacobianl) * weight * dble(rhostore_m2(i,j,k,ispec)),kind=CUSTOM_REAL)
                       endif !----------------------------------------------m2
                    enddo
                 enddo
              enddo
     ...
     endif ! lucas, end not the PML_CONDITIONS
  endif
  ...
  if (STACEY_ABSORBING_CONDITIONS) call create_mass_matrices_Stacey(nglob,nspec,ibool) !Lucas, have added for m2, CTD_SEM
...
! thus the mass matrix must be replaced by three mass matrices including the "C" damping matrix
  ...
  use generate_databases_par, only: DT,NGLLX,NGLLY,NGLLZ,CUSTOM_REAL,SIZE_REAL,NGLLSQUARE, CTD_SEM !lucas
  use create_regions_mesh_ext_par
  ...
  real(kind=CUSTOM_REAL) :: tx_m2,ty_m2,tz_m2 ! CTD_SEM
  ...
  if (ELASTIC_SIMULATION) then
    ...
    allocate(rmassx_m2(nglob_xy), rmassy_m2(nglob_xy), rmassz_m2(nglob_xy), stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 666 for m2')
    if (ier /= 0) stop 'error in allocate 21 for m2'
  ...
  do iface = 1,num_abs_boundary_faces
    ...
    if (ispec_is_elastic(ispec)) then
      ...
      do igll = 1,NGLLSQUARE
        ...
        if(CTD_SEM) then !lucas------------------------------
        ! C*deltat/2 contributions
        tx_m2 = rho_vp_m2(i,j,k,ispec)*vn*nx + rho_vs_m2(i,j,k,ispec)*(deltatover2-vn*nx)
        ty_m2 = rho_vp_m2(i,j,k,ispec)*vn*ny + rho_vs_m2(i,j,k,ispec)*(deltatover2-vn*ny)
        tz_m2 = rho_vp_m2(i,j,k,ispec)*vn*nz + rho_vs_m2(i,j,k,ispec)*(deltatover2-vn*nz)
        ...
        ! assembles mass matrix on global points
        rmassx_m2(iglob) = rmassx_m2(iglob) + tx_m2*jacobianw
        rmassy_m2(iglob) = rmassy_m2(iglob) + ty_m2*jacobianw
        rmassz_m2(iglob) = rmassz_m2(iglob) + tz_m2*jacobianw
        endif !----------------------------------------------
