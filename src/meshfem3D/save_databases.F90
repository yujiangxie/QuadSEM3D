...
!  character(len=MAX_STRING_LEN), dimension(6,1) :: undef_mat_prop
  !CTD_SEM, where the (7,1) used for tomography_xxx_m2.xyz
  character(len=MAX_STRING_LEN), dimension(7,1) :: undef_mat_prop ! CTD_SEM
  ...
  do i = 1,nundef ! lucas, before is NMATERIALS
    ...
    if (mat_id < 0) then
      ...
      case (IDOMAIN_ELASTIC) ! lucas IDOMAIN_ELASTIC=2 set in constant.h
        undef_mat_prop(3,1) = 'elastic'
      end select
      ! default name

     ! debug by lucas
      write(file_str,"(i2.2)") i
      !print *,'lucas defined file_i: ',file_str
      undef_mat_prop(4,1) = 'tomography_model_'//trim(file_str)//'.xyz' ! how to write vary file name 21-(i-1)
      ! debug by lucas
      !print *,'lucas defined tomo file name: ',undef_mat_prop(4,1)
      ! default tomo-id (unused)
      write(undef_mat_prop(5,1),*) 0
      ! domain-id
      write(undef_mat_prop(6,1),*) domain_id
      undef_mat_prop(7,1) = 'tomography_model_'//trim(file_str)//'_m2.xyz' ! CTD_SEM
      ! debug by lucas

      !print *,'lucas undef mat: ',undef_mat_prop
