...
!--------------------------------------------------------------------------------------------------
  ...
  !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: vp_tomography_m2,vs_tomography_m2,rho_tomography_m2,z_tomography_m2
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: qp_tomography_m2,qs_tomography_m2
  ! models entries
  ...
  double precision, dimension(:), allocatable :: VP_MIN_m2,VS_MIN_m2,RHO_MIN_m2,VP_MAX_m2,VS_MAX_m2,RHO_MAX_m2 !CTD_SEM
  ...
  logical,dimension(:),allocatable :: tomo_has_q_values_m2 ! CTD_SEM
...
!
  ...
  use generate_databases_par, only: CTD_SEM !lucas, CTD_SEM
  implicit none
...
! determines the number of external tomographic models and sets a total maximum number of element records
 ...
 !!!----------------------------------------------------- m2 -------------------------------------------
  ...
  subroutine init_tomography_files_m2()

! determines the number of external tomographic models and sets a total maximum number of element records

  use constants, only: MAX_STRING_LEN,IIN,IMAIN

  use generate_databases_par, only: TOMOGRAPHY_PATH_m2,undef_mat_prop,nundefMat_ext_mesh,IMODEL
  ...
  do iundef = 1, nmaterials
    ...
    if (nundefMat_ext_mesh == 0 .and. IMODEL == IMODEL_TOMO) then
      ...
      ! gets filename from material property
!      read(undef_mat_prop(4,iundef),*) filename
      read(undef_mat_prop(7,iundef),*) filename ! CTD_SEM
    ...
    ! sets filename with path (e.g. "DATA/tomo_files/" + "tomo.xyz")
    ! corrects the path and filename of tomography model
    if (TOMOGRAPHY_PATH_m2(len_trim(TOMOGRAPHY_PATH_m2):len_trim(TOMOGRAPHY_PATH_m2)) == "/") then
      tomo_filename = TOMOGRAPHY_PATH_m2(1:len_trim(TOMOGRAPHY_PATH_m2)) // trim(filename)
    else
      tomo_filename = TOMOGRAPHY_PATH_m2(1:len_trim(TOMOGRAPHY_PATH_m2)) // '/' // trim(filename)
    endif
    ...
    open(unit=IIN,file=trim(tomo_filename),status='old',action='read',iostat=ier)
    if (ier /= 0) then
      print *,'Error: could not open tomography file for m2: ',trim(tomo_filename)
      print *,'Please check your settings in Par_file ...'
      call exit_MPI(myrank_tomo,'error reading tomography file for m2')
    endif
    ...
    !print *,'tomography file: number of tokens on first data line: ',ntokens,' line: ',trim(string_read)
    if (ntokens /= 6 .and. ntokens /= 8) then
      print *,'Error reading tomography file, data line has wrong number of entries for m2: ',trim(string_read)
      stop 'Error reading tomography file for m2'
    endif
    ...
    if (nlines /= nrec .and. myrank_tomo == 0) then
       print *, 'Error: ',trim(tomo_filename),' has invalid number of records for m2'
       print *, '     number of grid points specified (= NX*NY*NZ) for m2:',nrec
       print *, '     number of file lines for grid points for m2        :',nlines
       stop 'Error in tomography data file for the grid points definition for m2'
    endif
  ...
  if (myrank_tomo == 0) then
    write(IMAIN,*)
    write(IMAIN,*) '     number of tomographic models for m2       = ',NFILES_TOMO
    write(IMAIN,*) '     maximum number of data records for m2     = ',nrecord_max
    write(IMAIN,*) '     size of required tomography arrays for m2 = ', &
      sngl( 4.d0 * NFILES_TOMO * nrecord_max * CUSTOM_REAL / 1024.d0 /1024.d0),'MB per process'
    write(IMAIN,*)
    call flush_IMAIN()
  endif
...
!  if (ier /= 0) call exit_MPI(myrank_tomo,'not enough memory to allocate tomo arrays')
  ...
  ! allocate models parameter records
  allocate(vp_tomography_m2(NFILES_TOMO,nrecord_max),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 856 for m2')
  allocate(vs_tomography_m2(NFILES_TOMO,nrecord_max),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 857 for m2')
  allocate(rho_tomography_m2(NFILES_TOMO,nrecord_max),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 858 for m2')
  allocate(z_tomography_m2(NFILES_TOMO,nrecord_max),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 859 for m2')
  if (ier /= 0) call exit_MPI(myrank_tomo,'not enough memory to allocate tomo arrays for m2')
  !lucas, the same as m1
  ! allocate models entries
!  allocate(NX(NFILES_TOMO),NY(NFILES_TOMO),NZ(NFILES_TOMO),stat=ier)
!  if (ier /= 0) call exit_MPI_without_rank('error allocating array 860')
...
!  if (ier /= 0) call exit_MPI(myrank_tomo,'not enough memory to allocate tomo arrays')
  ...
  ! allocate models min/max statistics
  allocate(VP_MIN_m2(NFILES_TOMO),VS_MIN_m2(NFILES_TOMO),RHO_MIN_m2(NFILES_TOMO),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 862 for m2')
  allocate(VP_MAX_m2(NFILES_TOMO),VS_MAX_m2(NFILES_TOMO),RHO_MAX_m2(NFILES_TOMO),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 863 for m2')
  if (ier /= 0) call exit_MPI(myrank_tomo,'not enough memory to allocate tomo arrays for m2')

  ! q values
  allocate(tomo_has_q_values_m2(NFILES_TOMO),stat=ier) !CTD_SEM
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 864 for m2')
  if (ier /= 0) call exit_MPI(myrank_tomo,'not enough memory to allocate tomo q-flag array for m2')
  tomo_has_q_values_m2(:) = .false. ! CTD_SEM
  ! stores data format flag
  do ifiles_tomo = 1,NFILES_TOMO
    tomo_has_q_values_m2(ifiles_tomo) = materials_with_q(ifiles_tomo) !CTD_SEM
  enddo
  deallocate(materials_with_q)

  ! only allocate q arrays if needed
  if (any(tomo_has_q_values_m2)) then  ! CTD_SEM
    allocate(qp_tomography_m2(NFILES_TOMO,nrecord_max),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 865 for m2')
    allocate(qs_tomography_m2(NFILES_TOMO,nrecord_max),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 866 for m2')
    if (ier /= 0) call exit_MPI(myrank_tomo,'not enough memory to allocate tomo q-value arrays for m2')
  endif

end subroutine init_tomography_files_m2
...
! read Vp Vs and rho from extracted text file
  ...
  !print *,'lucas to see nundefMat_ext_mesh: ',nundefMat_ext_mesh

  ! checks if we over-impose a tomography model by Par_file setting: MODEL = tomo
  if (nundefMat_ext_mesh == 0 .and. IMODEL == IMODEL_TOMO) then  ! not used in case of considering undefmat
    nmaterials = 1
  endif

  imat = 0
  do iundef = 1, nmaterials !! lucas, the first to read is 21, i.e., the top layer.
    ...
    if (has_q_values) then
      ! format: #x #y #z #vp #vs #density #Q_p #Q_s, lucas, here is Qp not Qkappa, but Qs=Qmu, Qp!=Qkappa
...
!!!----------------------------------------------------------- m2 ---------------------
  ...
  subroutine read_model_tomography_m2()

! read Vp Vs and rho from extracted text file

  use constants, only: MAX_STRING_LEN,IIN,IMAIN

  use generate_databases_par, only: TOMOGRAPHY_PATH_m2,undef_mat_prop,nundefMat_ext_mesh,IMODEL !lucas, CTD_SEM
...
!      read(undef_mat_prop(4,iundef),*) filename
    ...
    ! user output
    if (myrank_tomo == 0) then
       write(IMAIN,*) '     material id for m2: ',-imat
       write(IMAIN,*) '     file for m2      : ',trim(tomo_filename)
       call flush_IMAIN()
    endif

    ! opens file for reading
    open(unit=IIN,file=trim(tomo_filename),status='old',action='read',iostat=ier)
    if (ier /= 0) call exit_MPI(myrank_tomo,'Error opening tomography file for m2')
    ...
    ! format: #vp_min #vp_max #vs_min #vs_max #density_min #density_max
    call tomo_read_next_line(IIN,string_read)
    read(string_read,*) VP_MIN_m2(imat), VP_MAX_m2(imat), &  ! CTD_SEM
                        VS_MIN_m2(imat), VS_MAX_m2(imat), &
                        RHO_MIN_m2(imat), RHO_MAX_m2(imat)
    ...
    ! determines data format
    has_q_values = tomo_has_q_values_m2(imat) ! CTD_SEM

    ! user output
    if (myrank_tomo == 0) then
      if (has_q_values) then
        write(IMAIN,*) '     data format: #x #y #z #vp #vs #density #Q_p #Q_s for m2'
      else
        write(IMAIN,*) '     data format: #x #y #z #vp #vs #density for m2'
      endif
      call flush_IMAIN()
    endif
    ...
    if (has_q_values) then
      ! format: #x #y #z #vp #vs #density #Q_p #Q_s
      read(string_read,*) x_tomo,y_tomo,z_tomo,vp_tomo,vs_tomo,rho_tomo,qp_tomo,qs_tomo
      qp_tomography_m2(imat,1) = qp_tomo
      qs_tomography_m2(imat,1) = qs_tomo
    else
      ! format: #x #y #z #vp #vs #density
      read(string_read,*) x_tomo,y_tomo,z_tomo,vp_tomo,vs_tomo,rho_tomo
    endif

    ! stores record values, CTD_SEM
    vp_tomography_m2(imat,1) = vp_tomo
    vs_tomography_m2(imat,1) = vs_tomo
    rho_tomography_m2(imat,1) = rho_tomo
    z_tomography_m2(imat,1) = z_tomo
    ...
    if (has_q_values) then
      do irecord = 2,nrecord(imat)
        ! format: #x #y #z #vp #vs #density #Q_p #Q_s
        read(IIN,*,iostat=ier) x_tomo,y_tomo,z_tomo,vp_tomo,vs_tomo,rho_tomo,qp_tomo,qs_tomo
        if (ier /= 0) stop 'Error reading tomo file line format with q values for m2'

        ! stores record values
        vp_tomography_m2(imat,irecord) = vp_tomo
        vs_tomography_m2(imat,irecord) = vs_tomo
        rho_tomography_m2(imat,irecord) = rho_tomo
        z_tomography_m2(imat,irecord) = z_tomo
        qp_tomography_m2(imat,irecord) = qp_tomo
        qs_tomography_m2(imat,irecord) = qs_tomo
      enddo
    else
      do irecord = 2,nrecord(imat)
        ! format: #x #y #z #vp #vs #density
    ...
    ! user output
    if (myrank_tomo == 0) then
      write(IMAIN,*) '     number of grid points = NX*NY*NZ for m2:',nrecord(imat)
      write(IMAIN,*)
      call flush_IMAIN()
    endif
  enddo

  end subroutine read_model_tomography_m2
...
!
  ...
  spac_x = (xmesh - ORIG_X(imat)) / SPACING_X(imat) ! lucas, SPACING_X provied in tomography_model.xyz
  ...
  ! define 8 corners of interpolation element ! lucas, NX provided in tomography_model.xyz
  ...
  ! Vp
  vp1 = vp_tomography(imat,p0+1)! lucas, vp_tomography provided by tomography_model.xyz
...
!!======================================================================================== m2 
  subroutine model_tomography_m2(xmesh,ymesh,zmesh,rho_model,vp_model,vs_model, &
                              qkappa_atten,qmu_atten,imaterial_id, &
                              has_tomo_value)
!! call model_tomography_m2(xmesh,ymesh,zmesh,rho_m2,vp_m2,vs_m2,qkappa_atten_m2,qmu_atten_m2,imaterial_id,has_tomo_value_m2)    
  ...
  !CTD_SEM, changed for z_tomography_m2. This also does not need to change since z_tomography_m2 = z_tomography
  ! interpolation gamma factors
  if (z_tomography_m2(imat,p4+1) == z_tomography_m2(imat,p0+1)) then 
     gamma_interp_z1 = 1.d0
  else
     gamma_interp_z1 = (zmesh-z_tomography_m2(imat,p0+1))/(z_tomography_m2(imat,p4+1)-z_tomography_m2(imat,p0+1))
  endif
  if (gamma_interp_z1 > 1.d0) then
     gamma_interp_z1 = 1.d0
  endif
  ...
  if (z_tomography_m2(imat,p5+1) == z_tomography_m2(imat,p1+1)) then
     gamma_interp_z2 = 1.d0
  else
     gamma_interp_z2 = (zmesh-z_tomography_m2(imat,p1+1))/(z_tomography_m2(imat,p5+1)-z_tomography_m2(imat,p1+1))
  endif
  if (gamma_interp_z2 > 1.d0) then
     gamma_interp_z2 = 1.d0
  endif
  ...
  if (z_tomography_m2(imat,p6+1) == z_tomography_m2(imat,p2+1)) then
     gamma_interp_z3 = 1.d0
  else
     gamma_interp_z3 = (zmesh-z_tomography_m2(imat,p2+1))/(z_tomography_m2(imat,p6+1)-z_tomography_m2(imat,p2+1))
  endif
  if (gamma_interp_z3 > 1.d0) then
     gamma_interp_z3 = 1.d0
  endif
  ...
  if (z_tomography_m2(imat,p7+1) == z_tomography_m2(imat,p3+1)) then
     gamma_interp_z4 = 1.d0
  else
     gamma_interp_z4 = (zmesh-z_tomography_m2(imat,p3+1))/(z_tomography_m2(imat,p7+1)-z_tomography_m2(imat,p3+1))
  endif
  if (gamma_interp_z4 > 1.d0) then
     gamma_interp_z4 = 1.d0
  endif
  ...
  ! Vp, CTD_SEM
  vp1 = vp_tomography_m2(imat,p0+1)! lucas, vp_tomography provided by tomography_model.xyz
  vp2 = vp_tomography_m2(imat,p1+1)
  vp3 = vp_tomography_m2(imat,p2+1)
  vp4 = vp_tomography_m2(imat,p3+1)
  vp5 = vp_tomography_m2(imat,p4+1)
  vp6 = vp_tomography_m2(imat,p5+1)
  vp7 = vp_tomography_m2(imat,p6+1)
  vp8 = vp_tomography_m2(imat,p7+1)
  ! use trilinear interpolation in cell to define Vp
  vp_final = interpolate_trilinear(vp1,vp2,vp3,vp4,vp5,vp6,vp7,vp8)

  ! Vs, CTD_SEM
  vs1 = vs_tomography_m2(imat,p0+1)
  vs2 = vs_tomography_m2(imat,p1+1)
  vs3 = vs_tomography_m2(imat,p2+1)
  vs4 = vs_tomography_m2(imat,p3+1)
  vs5 = vs_tomography_m2(imat,p4+1)
  vs6 = vs_tomography_m2(imat,p5+1)
  vs7 = vs_tomography_m2(imat,p6+1)
  vs8 = vs_tomography_m2(imat,p7+1)
  ! use trilinear interpolation in cell to define Vs
  vs_final = interpolate_trilinear(vs1,vs2,vs3,vs4,vs5,vs6,vs7,vs8)

  ! density, CTD_SEM
  rho1 = rho_tomography_m2(imat,p0+1)
  rho2 = rho_tomography_m2(imat,p1+1)
  rho3 = rho_tomography_m2(imat,p2+1)
  rho4 = rho_tomography_m2(imat,p3+1)
  rho5 = rho_tomography_m2(imat,p4+1)
  rho6 = rho_tomography_m2(imat,p5+1)
  rho7 = rho_tomography_m2(imat,p6+1)
  rho8 = rho_tomography_m2(imat,p7+1)
  ! use trilinear interpolation in cell to define rho
  rho_final = interpolate_trilinear(rho1,rho2,rho3,rho4,rho5,rho6,rho7,rho8)

  ! impose minimum and maximum velocity and density if needed
  if (vp_final < VP_MIN_m2(imat)) vp_final = VP_MIN_m2(imat)
  if (vp_final > VP_MAX_m2(imat)) vp_final = VP_MAX_m2(imat)

  if (vs_final < VS_MIN_m2(imat)) vs_final = VS_MIN_m2(imat)
  if (vs_final > VS_MAX_m2(imat)) vs_final = VS_MAX_m2(imat)

  if (rho_final > RHO_MAX_m2(imat)) rho_final = RHO_MAX_m2(imat)
  if (rho_final < RHO_MIN_m2(imat)) rho_final = RHO_MIN_m2(imat)
  ...
  ! attenuation, CTD_SEM
  if (tomo_has_q_values(imat)) then
    qp1 = qp_tomography_m2(imat,p0+1)
    qp2 = qp_tomography_m2(imat,p1+1)
    qp3 = qp_tomography_m2(imat,p2+1)
    qp4 = qp_tomography_m2(imat,p3+1)
    qp5 = qp_tomography_m2(imat,p4+1)
    qp6 = qp_tomography_m2(imat,p5+1)
    qp7 = qp_tomography_m2(imat,p6+1)
    qp8 = qp_tomography_m2(imat,p7+1)
    ! use trilinear interpolation in cell
    qp_final = interpolate_trilinear(qp1,qp2,qp3,qp4,qp5,qp6,qp7,qp8)

    qs1 = qs_tomography_m2(imat,p0+1)
    qs2 = qs_tomography_m2(imat,p1+1)
    qs3 = qs_tomography_m2(imat,p2+1)
    qs4 = qs_tomography_m2(imat,p3+1)
    qs5 = qs_tomography_m2(imat,p4+1)
    qs6 = qs_tomography_m2(imat,p5+1)
    qs7 = qs_tomography_m2(imat,p6+1)
    qs8 = qs_tomography_m2(imat,p7+1)
    ! use trilinear interpolation in cell
    qs_final = interpolate_trilinear(qs1,qs2,qs3,qs4,qs5,qs6,qs7,qs8)
  ...
  end subroutine model_tomography_m2
...
!
  ...
  subroutine deallocate_tomography_files()
    ...
    use model_tomography_par
    use generate_databases_par, only: CTD_SEM ! CTD_SEM
    ...
    if(CTD_SEM) then !------
    deallocate(vp_tomography_m2)
    deallocate(vs_tomography_m2)
    deallocate(rho_tomography_m2)
    deallocate(z_tomography_m2)
    endif !-----------------
    ...
    if(CTD_SEM) then !------
    deallocate(VP_MIN_m2,VS_MIN_m2,RHO_MIN_m2)
    deallocate(VP_MAX_m2,VS_MAX_m2,RHO_MAX_m2)
    endif !-----------------
    ...
    if(CTD_SEM) then !------
    if (any(tomo_has_q_values_m2)) then
      deallocate(qp_tomography_m2)
      deallocate(qs_tomography_m2)
    endif
    deallocate(tomo_has_q_values_m2)  
    endif !-----------------
