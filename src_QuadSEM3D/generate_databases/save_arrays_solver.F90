...
! for external mesh
  ...
  write(IOUT) mustore
  if(CTD_SEM) then !------
  write(IOUT) kappastore_m2
  write(IOUT) mustore_m2
  endif !------
...
! thus we allocate it and read it in all cases (whether the simulation is acoustic, elastic, or acoustic/elastic)
  write(IOUT) rhostore
  if(CTD_SEM) then !------
  write(IOUT) rhostore_m2
  endif !------

! elastic
  if (ELASTIC_SIMULATION) then
    write(IOUT) rmass
    if(CTD_SEM) then !------ used in pml
     write(IOUT) rmass_m2
    endif !-----------------
    ...
    write(IOUT) rho_vs
    if(CTD_SEM) then !------
    write(IOUT) rho_vp_m2
    write(IOUT) rho_vs_m2
    endif !------
  endif
    ...
    if (STACEY_ABSORBING_CONDITIONS .and. (.not. PML_CONDITIONS)) then
      ...
      if (ELASTIC_SIMULATION) then
        ...
        if(CTD_SEM) then ! lucas ------
         write(IOUT) rmassx_m2
         write(IOUT) rmassy_m2
         write(IOUT) rmassz_m2
        endif !------------------------
      endif
      if (ACOUSTIC_SIMULATION) then
...
! inner/outer elements
  ...
  if (SAVE_MESH_FILES) call save_arrays_solver_files(nspec,nglob,ibool) !lucas, CTD_SEM
...
! outputs binary files for single mesh parameters (for example vp, vs, rho, ..)
  ...
  use generate_databases_par, only: myrank,NGLLX,NGLLY,NGLLZ,NGLLSQUARE,IMAIN,IOUT,FOUR_THIRDS,CTD_SEM !lucas
  ...
  call write_VTK_data_gll_cr(nspec,nglob, &
                      ...
                      v_tmp,filename) ! lucas v_tmp is the variable to be wroten
...
!  ! frees temporary array, lucas, need to be used below
!  deallocate(v_tmp)
...
!  if (ier /= 0) call exit_MPI_without_rank('error allocating array')
  ...
  !endif
  v_tmp = 0.0
  where( rho_vp_m2 /= 0._CUSTOM_REAL ) v_tmp = (FOUR_THIRDS * mustore_m2 + kappastore_m2) / rho_vp_m2
  open(unit=IOUT,file=prname(1:len_trim(prname))//'vp_m2.bin',status='unknown',form='unformatted',iostat=ier)
  if (ier /= 0) stop 'error opening file vp_m2.bin'
  write(IOUT) v_tmp
  close(IOUT)

  ! vp values - VTK file output
  filename = prname(1:len_trim(prname))//'vp_m2'
  call write_VTK_data_gll_cr(nspec,nglob, &
                      xstore_dummy,ystore_dummy,zstore_dummy,ibool, &
                      v_tmp,filename) ! lucas v_tmp is the variable to be wroten
  ...
  !endif
  v_tmp = 0.0
  where( rho_vs_m2 /= 0._CUSTOM_REAL )  v_tmp = mustore_m2 / rho_vs_m2
  open(unit=IOUT,file=prname(1:len_trim(prname))//'vs_m2.bin',status='unknown',form='unformatted',iostat=ier)
  if (ier /= 0) stop 'error opening file vs_m2.bin'
  write(IOUT) v_tmp
  close(IOUT)

  ! vs values - VTK file output
  filename = prname(1:len_trim(prname))//'vs_m2'
  call write_VTK_data_gll_cr(nspec,nglob, &
                      xstore_dummy,ystore_dummy,zstore_dummy,ibool, &
                      v_tmp,filename)
  ...
  ! outputs density model for check
  v_tmp = 0.0
  where( rho_vp_m2 /= 0._CUSTOM_REAL ) v_tmp = rho_vp_m2**2 / (FOUR_THIRDS * mustore_m2 + kappastore_m2)
  open(unit=IOUT,file=prname(1:len_trim(prname))//'rho_m2.bin',status='unknown',form='unformatted',iostat=ier)
  if (ier /= 0) stop 'error opening file rho_m2.bin'
  write(IOUT) v_tmp
  close(IOUT)

  ! attenuation
  ! shear attenuation Qmu
  open(unit=IOUT,file=prname(1:len_trim(prname))//'qmu_m2.bin',status='unknown',form='unformatted',iostat=ier)
  if (ier /= 0) stop 'error opening file qmu_m2.bin'
  write(IOUT) qmu_attenuation_store_m2
  close(IOUT)

  ! shear attenuation - VTK file output
  filename = prname(1:len_trim(prname))//'qmu_m2'
  call write_VTK_data_gll_cr(nspec,nglob, &
                      xstore_dummy,ystore_dummy,zstore_dummy,ibool, &
                      qmu_attenuation_store_m2,filename)

  ! bulk attenuation Qkappa
  open(unit=IOUT,file=prname(1:len_trim(prname))//'qkappa_m2.bin',status='unknown',form='unformatted',iostat=ier)
  if (ier /= 0) stop 'error opening file qkappa_m2.bin'
  write(IOUT) qkappa_attenuation_store_m2
  close(IOUT)

  ! bulk attenuation - VTK file output
  filename = prname(1:len_trim(prname))//'qkappa_m2'
  call write_VTK_data_gll_cr(nspec,nglob, &
                      xstore_dummy,ystore_dummy,zstore_dummy,ibool, &
                      qkappa_attenuation_store_m2,filename)
