...
!=====================================================================
  ...
  use create_regions_mesh_ext_par !lucas, include rhostore,kappastore,mustore
  ...
  real(kind=CUSTOM_REAL) :: vp_m2,vs_m2,rho_m2,qkappa_atten_m2,qmu_atten_m2 ! lucas, CTD_SEM
  ...
  if (nundefMat_ext_mesh > 0 .or. IMODEL == IMODEL_TOMO) call model_tomography_broadcast(myrank) ! lucas, if IMODEL == IMODEL_TOMO or provided tomography_model_xyz files, read_model_tomogrpahy, e.g., get vp_tomography of each GLL point

  ! prepares external model values if needed
  select case (IMODEL)
  case (IMODEL_USER_EXTERNAL)
    call model_external_broadcast(myrank) !lucas, nothing here
...
! DP: not sure if this here should check if (COUPLE_WITH_INJECTION_TECHNIQUE .or. MESH_A_CHUNK_OF_THE_EARTH) ..
    ...
    do k = 1, NGLLZ
      do j = 1, NGLLY
        do i = 1, NGLLX
          ...
          ! assigns material properties. Lucas, to get model material properties (like vp, vs, rho) by different methods setting by IMODEL
          call get_model_values(mat_prop,nmat_ext_mesh, &
                                undef_mat_prop,nundefMat_ext_mesh, &
                                imaterial_id,imaterial_def, &
                                xmesh,ymesh,zmesh, &
                                rho,vp,vs,qkappa_atten,qmu_atten,idomain_id, &
                                rho_m2,vp_m2,vs_m2,qkappa_atten_m2,qmu_atten_m2, & ! CTD_SEM, only when at CTD_SEM, these values are used
                                rho_s,kappa_s,rho_f,kappa_f,eta_f,kappa_fr,mu_fr, &
                                phi,tort,kxx,kxy,kxz,kyy,kyz,kzz, &
                                c11,c12,c13,c14,c15,c16, &
                                c22,c23,c24,c25,c26,c33, &
                                c34,c35,c36,c44,c45,c46,c55,c56,c66, &
                                ANISOTROPY) !!! lucas, get rho,vp,vs,qkappa_atten,qmu_atten by this function, these 5 valuses are obtained by interpolation from tomogrpahy_model.xyz.
          ...
          if (idomain_id == IDOMAIN_ACOUSTIC .or. idomain_id == IDOMAIN_ELASTIC) then
            ...
            ! kappa, mu
            kappastore(i,j,k,ispec) = rho*( vp*vp - FOUR_THIRDS*vs*vs ) ! lucas FOUR_THIRDS =4_custom_real/3
            ...
            ! Stacey, a completer par la suite (Lucas translation: Stacey, to be completed later)
            ...
            ! elastic or acoustic material
            ! density
            rhostore_m2(i,j,k,ispec) = rho_m2
            ! kappa, mu
            kappastore_m2(i,j,k,ispec) = rho_m2*( vp_m2*vp_m2 - FOUR_THIRDS*vs_m2*vs_m2 ) ! lucas FOUR_THIRDS =4_custom_real/3
            mustore_m2(i,j,k,ispec) = rho_m2*vs_m2*vs_m2
            ! attenuation
            qkappa_attenuation_store_m2(i,j,k,ispec) = qkappa_atten_m2
            qmu_attenuation_store_m2(i,j,k,ispec) = qmu_atten_m2
            ! Stacey, a completer par la suite (Lucas translation: Stacey, to be completed later)
            rho_vp_m2(i,j,k,ispec) = rho_m2*vp_m2
            rho_vs_m2(i,j,k,ispec) = rho_m2*vs_m2
            endif !-------------------------------
          else
  ...
  if ( (nundefMat_ext_mesh > 0 .or. IMODEL == IMODEL_TOMO) ) call deallocate_tomography_files() !lucas, has done for m2, CTD_SEM
...
!
  ...
  subroutine get_model_values(mat_prop,nmat_ext_mesh, &
                              ...
                              xmesh,ymesh,zmesh, &
                              rho,vp,vs,qkappa_atten,qmu_atten,idomain_id, &
                              rho_m2,vp_m2,vs_m2,qkappa_atten_m2,qmu_atten_m2, & ! CTD_SEM, only when at CTD_SEM, these values are used and got by model_tomography_m2
                              rho_s,kappa_s,rho_f,kappa_f,eta_f,kappa_fr,mu_fr, &
                              phi,tort,kxx,kxy,kxz,kyy,kyz,kzz, &
                              c11,c12,c13,c14,c15,c16, &
                              c22,c23,c24,c25,c26,c33, &
  ...
  real(kind=CUSTOM_REAL) :: vp,vs,rho,qkappa_atten,qmu_atten
  real(kind=CUSTOM_REAL) :: vp_m2,vs_m2,rho_m2,qkappa_atten_m2,qmu_atten_m2 ! CTD_SEM
  ...
  logical :: has_tomo_value_m2 ! CTD_SEM
...
! *********************************************************************************
  ...
  case (IMODEL_TOMO)
!    print *, 'lucas test: read model from tomography_model_**.xyz'
    ! gets model values from tomography file
    call model_tomography(xmesh,ymesh,zmesh,rho,vp,vs,qkappa_atten,qmu_atten,imaterial_id,has_tomo_value)!!! lucas, get rho,vp,vs,qkappa_atten,qmu_atten from tomography_model.xyz (by trilinear interpolate in model_tomography subrontine.)
    if(CTD_SEM) then ! ------
