...
! takes model values specified by mesh properties
  ...
  if (imaterial_id > 0) then
    ...
    ! material domain_id
    idomain_id = nint(mat_prop(7,imaterial_id)) ! lucas, mat_prop is one line (row) defined the number of materials.
  ...
  else ! lucas ! negative material ids
    ...
    case (2)
      ...
      print *,'lucas test: model_tomography() in model_default()'
...
!      if(CTD_SEM) then ! ------! lucas, this one used with MODEL=default,
!      current implementation uses MODEL=tomo
