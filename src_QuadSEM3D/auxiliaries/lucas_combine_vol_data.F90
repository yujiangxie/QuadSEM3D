...
!=====================================================================
  ...
  program combine_vol_data ! lucas have chnaged this to show vp_tomo - vp_1D
...
! works for external, irregular meshes
  ...
  double precision,dimension(:,:,:,:),allocatable :: data_dp,data_dp_tomo !lucas
  ! real array for data
  real,dimension(:,:,:,:),allocatable :: data_sp,data_diff,data_sp_tomo !lucas
  ...
  integer :: numpoin
  !real    :: lucas_tomo,lucas_1D,lucas_diff
  integer :: i, it, ier
  integer :: iproc, num_node !ilucas,il,jl,kl !lucas
  ...
  character(len=MAX_STRING_LEN*2) :: mesh_file,local_data_file,local_data_file_tomo !lucas
...
!! DK DK remark: then why do you use MPI??? you could remove MPI entirely
  ...
  do it = 1, num_node
    ...
    !lucas times 4 for the depth for visualzition 
    zstore=zstore*4;
   ...
   !lucas ------------------
        allocate(data_sp_tomo(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
    if (ier /= 0) call my_local_exit_MPI_without_rank('error allocating array 1143')
    if (ier /= 0) stop 'error allocating single precision data array'
    if (CUSTOM_REAL == SIZE_DOUBLE) then
      allocate(data_dp_tomo(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
      if (ier /= 0) call my_local_exit_MPI_without_rank('error allocating array 1144')
      if (ier /= 0) stop 'error allocating double precision data array'
    endif
    if (CUSTOM_REAL == SIZE_DOUBLE) then
      allocate(data_diff(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
      if (ier /= 0) call my_local_exit_MPI_without_rank('error allocating array 1144')
      if (ier /= 0) stop 'error allocating double precision data array'
    endif
   !-------------------------
   ...
   ! Lucas, additional vtk files---------------------------
    write(prname,'(a,i6.6,a)') trim(indir)//'proc',iproc,'_'
   ! local_data_file_tomo = trim(prname) // trim(filename) //'_tomo' // '.bin'         !lucas2, to see the dvp, just read 2 sets of vp files here, and also change the lucas* code below
    local_data_file_tomo = trim(prname) // 'hess_kernel' // '.bin' 
    open(unit = 29,file = trim(local_data_file_tomo),status='old', &
          action='read',form ='unformatted',iostat=ier)
    if (ier /= 0) then
      print *,'Error opening ',trim(local_data_file_tomo)
      stop
    endif
   ...
   !lucas4, read vp_tomo-----------------------------------
   if (CUSTOM_REAL == SIZE_DOUBLE) then             ! lucas3, such as 28 for 1D vp model, and 29 for tomo vp model.
      read(29) data_dp_tomo
    else
      read(29) data_sp_tomo
    endif
    close(29)
    print *, trim(local_data_file_tomo)
   ...
   !lucas5,-------------------------------------------------
    ! uses conversion to real values
    if (CUSTOM_REAL == SIZE_DOUBLE) then
      data_sp_tomo(:,:,:,:) = sngl(data_dp_tomo(:,:,:,:))
      deallocate(data_dp_tomo)
    endif
    print *, trim(local_data_file_tomo)
     !lucas_tomo=0.0
     !lucas_1D=0.0
     !lucas_diff=0.0
     !lucas6, make the difference-----------------------------
     !do ilucas =1, NSPEC_AB
     ! do il=1,NGLLX
     !   do jl=1,NGLLY
     !    do kl=1,NGLLZ  
     !       lucas_tomo=data_sp_tomo(il,jl,kl,ilucas) 
     !       lucas_1D=data_sp(il,jl,kl,ilucas)
     !       lucas_diff=lucas_tomo - lucas_1D
     !       data_diff(il,jl,kl,ilucas)=lucas_diff
     !       lucas_tomo=0.0
     !       lucas_1D=0.0
     !       lucas_diff=0.0
     !    enddo
     ...
     ! enddo
     !enddo
     ! data_diff = data_sp_tomo - data_sp
      data_diff = data_sp*(1/data_sp_tomo)

     deallocate(data_sp_tomo)!lucas

    ! writes point coordinates and scalar value to mesh file
    if (.not. HIGH_RESOLUTION_MESH) then
      ! writes out element corners only
      call cvd_write_corners(NSPEC_AB,NGLOB_AB,ibool,xstore,ystore,zstore,data_diff,it,npp,numpoin,np) !lucas chnage data_sp to data_diff
    else
      ! high resolution, all GLL points
      call cvd_write_GLL_points(NSPEC_AB,NGLOB_AB,ibool,xstore,ystore,zstore,data_diff,it,npp,numpoin,np) !lucas chnage data_sp to data_diff
    endif
...
! writes element information
  ...
  print *, 'Done writing (vp_tomo - vp_1D) '//trim(mesh_file) !lucas, if need the orignal, just use the combine_vol_data_original.f90
