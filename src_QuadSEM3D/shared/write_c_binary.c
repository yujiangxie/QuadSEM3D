...
// reads binary file data in chunks of MAX_B, lucas, where MAX_B=16384
...
void read_abs_map(int *fid, char *buffer, int *length , int *index){
  ...
  // offset in bytes
  offset =  ((long long)*index -1) * (*length) ; // lucas, the length remaining
...
#ifdef   USE_MAP_FUNCTION
  write_abs_map(fid,buffer,length,index); //lucas. copy buffer to map[fid]
#else
...
#ifdef   USE_MAP_FUNCTION
  read_abs_map(fid,buffer,length,index);//lucas, copy map[fid] to buffer
#else
