...
! CTD_SEM ======
    call read_value_logical(CTD_SEM, 'CTD_SEM', ier)   
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'CTD_SEM                       = .false.'
      write(*,*)
    endif

    call read_value_logical(compute_approx_Hessian, 'compute_approx_Hessian', ier)   
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'compute_approx_Hessian        = .false.'
      write(*,*)
    endif

    call read_value_string(TOMOGRAPHY_PATH_m2, 'TOMOGRAPHY_PATH_m2', ier)
    if (ier /= 0) then
      some_parameters_missing_from_Par_file = .true.
      write(*,'(a)') 'TOMOGRAPHY_PATH_m2                 = ./DATA/tomo_files_m2/'
      write(*,*)
    endif
! CTD_SEM ======
...
!! DK DK remove leading ./ if any, Paul Cristini said it could lead to problems when NUMBER_OF_SIMULTANEOUS_RUNS > 1
      ...
      !CTD_SEM, !------
      TOMOGRAPHY_PATH_new_m2 = adjustl(TOMOGRAPHY_PATH_m2)
      if (index (TOMOGRAPHY_PATH_new_m2, './') == 1) then
        TOMOGRAPHY_PATH_m2 = TOMOGRAPHY_PATH_new_m2(3:)
      endif
      !----------------
      ...
      !CTD_SEM---
      TOMOGRAPHY_PATH_m2 = path_to_add(1:len_trim(path_to_add))//TOMOGRAPHY_PATH_m2(1:len_trim(TOMOGRAPHY_PATH_m2))
      !----------
  ...
  if (BROADCAST_AFTER_READ) then
    ...
    !CTD_SEM---  
    call bcast_all_singlel(CTD_SEM)
    call bcast_all_singlel(compute_approx_Hessian)
    call bcast_all_string(TOMOGRAPHY_PATH_m2)
    !---
