...
!----
  !lucas, sum the array_val of each ibool of each iinterface togather, i.e., sum the array_val of all GLL points of all interfaces of the partition border together. 
  subroutine assemble_MPI_scalar_blocking(NPROC,NGLOB_AB,array_val, &
                        num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                        nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                        my_neighbors_ext_mesh) ! lucas, shoud to check the my_neighbors_ext_mesh? corresponding to nibool of each interface?
  ...
  if (NPROC > 1) then
    ...
    do iinterface = 1, num_interfaces_ext_mesh
      ...
      call isend_cr(buffer_send_scalar(1:nibool_interfaces_ext_mesh(iinterface),iinterface), &
                     ...
                     itag, &
                     request_send_scalar(iinterface)) ! lucas, buffer_send_scalar(1:nibool_interfaces_ext_mesh(iinterface),iinterface) indicates process rank. 
                                                      ! lucas, send from buffer_send_scalar to my_neighbors_ext_mesh.
      ! receive request
      call irecv_cr(buffer_recv_scalar(1:nibool_interfaces_ext_mesh(iinterface),iinterface), &
                     nibool_interfaces_ext_mesh(iinterface), &
                     my_neighbors_ext_mesh(iinterface), &
                     itag, &
                     request_recv_scalar(iinterface)) ! lucas,from my_neighbors_ext_mesh to buffer_recv_scalar.
    enddo
