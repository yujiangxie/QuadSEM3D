...
!
  ...
  subroutine sum_all_1Darray_dp(sendbuf, recvbuf, nx) !lucas, nx=number of elements in send buffer
...
! asynchronuous send/receive
  ...
  subroutine isend_cr(sendbuf, sendcount, dest, sendtag, req)!lucas, from sendbuf to dest
  ...
  call MPI_ISEND(sendbuf,sendcount,CUSTOM_MPI_TYPE,dest,sendtag,my_local_mpi_comm_world,req,ier)!lucas, from sendbuf to dest
...
!
  ...
  subroutine irecv_cr(recvbuf, recvcount, dest, recvtag, req)!lucas, from dest to recvbuf
  ...
  call MPI_IRECV(recvbuf,recvcount,CUSTOM_MPI_TYPE,dest,recvtag,my_local_mpi_comm_world,req,ier)!lucas, from dest to recvbuf
