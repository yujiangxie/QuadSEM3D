...
! United States and French Government Sponsorship Acknowledged.
  ...
  if(CTD_SEM .and. UNDO_ATTENUATION_AND_OR_PML) then !---------m2
  call prepare_attenuation_m2()
   if(compute_approx_Hessian) then !-------m1
   call prepare_attenuation_m1()
   endif !---------------------------------m1
  endif !------------------------------------------------------m2
...
!
  ...
  !lucas 2, the CTD_SEM right now only used for elastic case and Newmark scheme
  if (ELASTIC_SIMULATION) then
    ...
    if (STACEY_ABSORBING_CONDITIONS) then
      if (USE_LDDRK) then
        ...
        rmassy(:) = rmass(:) + rmassy(:)
        rmassz(:) = rmass(:) + rmassz(:)
        if(CTD_SEM) then !-----------------------m2, rmass_m2 =0 saved from databases.bin
        rmassx_m2(:) = rmass_m2(:) + rmassx_m2(:)
        rmassy_m2(:) = rmass_m2(:) + rmassy_m2(:)
        rmassz_m2(:) = rmass_m2(:) + rmassz_m2(:)
        endif !----------------------------------m2
    ...
    if(CTD_SEM) then !------------------------------------------------------------------------m2
    ! assemble mass matrix
    call assemble_MPI_scalar_blocking(NPROC,NGLOB_AB,rmassx_m2, &
                        num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                        nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                        my_neighbors_ext_mesh)
    call assemble_MPI_scalar_blocking(NPROC,NGLOB_AB,rmassy_m2, &
                        num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                        nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                        my_neighbors_ext_mesh)
    call assemble_MPI_scalar_blocking(NPROC,NGLOB_AB,rmassz_m2, &
                        num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                        nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                        my_neighbors_ext_mesh)

    ! fill mass matrix with fictitious non-zero values to make sure it can be inverted globally
    where(rmassx_m2 <= 0._CUSTOM_REAL) rmassx_m2 = 1._CUSTOM_REAL
    where(rmassy_m2 <= 0._CUSTOM_REAL) rmassy_m2 = 1._CUSTOM_REAL
    where(rmassz_m2 <= 0._CUSTOM_REAL) rmassz_m2 = 1._CUSTOM_REAL
    rmassx_m2(:) = 1._CUSTOM_REAL / rmassx_m2(:)
    rmassy_m2(:) = 1._CUSTOM_REAL / rmassy_m2(:)
    rmassz_m2(:) = 1._CUSTOM_REAL / rmassz_m2(:)
    endif !------------------------------------------------------------------------------------m2
...
!
  ...
  subroutine prepare_timerun_lddrk() !lucas, FIX_UNDERFLOW_PROBLEM = .false.
...
! prepares adjoint simulations
  ...
  if (ATTENUATION .and. SIMULATION_TYPE == 3) then
    ...
    if(CTD_SEM) then !----------------------------------------------------------------------m2
     !lucas, tau_sigma_m2 depends on min_resolved_period_m2 (vp relevant), see get_attenuation_constants() in prepare_attenuation.f90 for details
     call get_attenuation_memory_values(tau_sigma_m2,b_deltat,b_alphaval_m2,b_betaval_m2,b_gammaval_m2) 
    endif !---------------------------------------------------------------------------------m2
  endif
  ...
  if (SIMULATION_TYPE == 3) then
    ...
    if (ELASTIC_SIMULATION) then
      ...
      if (ANISOTROPIC_KL) then
        ...
        if(CTD_SEM) then !-------------------m2
        mu_kl_m2(:,:,:,:)    = 0._CUSTOM_REAL
        kappa_kl_m2(:,:,:,:) = 0._CUSTOM_REAL
        endif !------------------------------m2
      endif
      ...
      if (CTD_SEM) then !-----------------------------------------------------------------m2
      rho_kl_Ha(:,:,:,:)   = 0._CUSTOM_REAL
      rho_kl_Hbm(:,:,:,:)   = 0._CUSTOM_REAL
      if(compute_approx_Hessian) then !------
       rho_kl_Hbs(:,:,:,:)   = 0._CUSTOM_REAL
      endif !--------------------------------
      rho_kl_Hc(:,:,:,:)   = 0._CUSTOM_REAL
      rho_kl_Habc(:,:,:,:)   = 0._CUSTOM_REAL
      rho_kl_Hdiff(:,:,:,:)   = 0._CUSTOM_REAL

!      if (ANISOTROPIC_KL) then
!        cijkl_kl(:,:,:,:,:) = 0._CUSTOM_REAL
!      else
        mu_kl_Ha(:,:,:,:)    = 0._CUSTOM_REAL
        kappa_kl_Ha(:,:,:,:) = 0._CUSTOM_REAL
        mu_kl_Hbm(:,:,:,:)    = 0._CUSTOM_REAL
        kappa_kl_Hbm(:,:,:,:) = 0._CUSTOM_REAL
        if(compute_approx_Hessian) then !---------
         mu_kl_Hbs(:,:,:,:)    = 0._CUSTOM_REAL
         kappa_kl_Hbs(:,:,:,:) = 0._CUSTOM_REAL
        endif !-----------------------------------
        mu_kl_Hc(:,:,:,:)    = 0._CUSTOM_REAL
        kappa_kl_Hc(:,:,:,:) = 0._CUSTOM_REAL
        mu_kl_Habc(:,:,:,:)    = 0._CUSTOM_REAL
        kappa_kl_Habc(:,:,:,:) = 0._CUSTOM_REAL
        mu_kl_Hdiff(:,:,:,:)    = 0._CUSTOM_REAL
        kappa_kl_Hdiff(:,:,:,:) = 0._CUSTOM_REAL
!      endif
...
!      endif
      ...
      ! reconstructed/backward elastic wavefields
      b_displ_m2 = 0._CUSTOM_REAL
      b_veloc_m2 = 0._CUSTOM_REAL
      b_accel_m2 = 0._CUSTOM_REAL
      if (FIX_UNDERFLOW_PROBLEM) b_displ_m2 = VERYSMALLVAL

      ! memory variables if attenuation
      if (ATTENUATION) then
        b_R_trace_m2 = 0._CUSTOM_REAL
        b_R_xx_m2 = 0._CUSTOM_REAL
        b_R_yy_m2 = 0._CUSTOM_REAL
        b_R_xy_m2 = 0._CUSTOM_REAL
        b_R_xz_m2 = 0._CUSTOM_REAL
        b_R_yz_m2 = 0._CUSTOM_REAL
        b_epsilondev_trace_m2 = 0._CUSTOM_REAL
        b_epsilondev_xx_m2 = 0._CUSTOM_REAL
        b_epsilondev_yy_m2 = 0._CUSTOM_REAL
        b_epsilondev_xy_m2 = 0._CUSTOM_REAL
        b_epsilondev_xz_m2 = 0._CUSTOM_REAL
        b_epsilondev_yz_m2 = 0._CUSTOM_REAL
      endif
  ...
  if (STACEY_ABSORBING_CONDITIONS) then
    ...
    if (num_abs_boundary_faces > 0 .and. (SIMULATION_TYPE == 3 .or. &
      ...
      if (ELASTIC_SIMULATION) then
        ...
        if(CTD_SEM) then !------------------------------------------------------------------m2
        ! allocates wavefield
        allocate(b_absorb_field_m2(NDIM,NGLLSQUARE,b_num_abs_boundary_faces),stat=ier)
        if (ier /= 0) call exit_MPI_without_rank('error allocating array 2143 for m2')
        if (ier /= 0) stop 'error allocating array b_absorb_field_m2'
         if(compute_approx_Hessian) then !-------m1
         allocate(b_absorb_field_m1(NDIM,NGLLSQUARE,b_num_abs_boundary_faces),stat=ier)
         if (ier /= 0) call exit_MPI_without_rank('error allocating array 2143 for m1')
         if (ier /= 0) stop 'error allocating array b_absorb_field_m1'
         endif !---------------------------------m1

         ! lucas, this part is repeat from above for m1
!        ! size of single record
!        b_reclen_field_m2 = CUSTOM_REAL * NDIM * NGLLSQUARE * num_abs_boundary_faces

!        ! check integer size limit: size of b_reclen_field must fit onto an 4-byte integer
!        if (num_abs_boundary_faces > 2147483646 / (CUSTOM_REAL * NDIM * NGLLSQUARE)) then
!          print *,'reclen needed exceeds integer 4-byte limit: ',b_reclen_field_m2
!          print *,'  ',CUSTOM_REAL, NDIM, NGLLSQUARE, num_abs_boundary_faces
!          print *,'bit size Fortran: ',bit_size(b_reclen_field_m2)
!          call exit_MPI(myrank,"error b_reclen_field_m2 integer limit")
!        endif

!        ! total file size
!        filesize = b_reclen_field_m2
!        filesize = filesize * NSTEP

        if (SIMULATION_TYPE == 3) then
          ! opens existing files
          call open_file_abs_r(IOABS_m2,trim(prname)//'absorb_field_m2.bin', & ! lucas add for m2
                              len_trim(trim(prname)//'absorb_field_m2.bin'), &
                              filesize)
        else
          ! opens new file
          call open_file_abs_w(IOABS_m2,trim(prname)//'absorb_field_m2.bin', &
                              len_trim(trim(prname)//'absorb_field_m2.bin'), &
                              filesize)
        endif
        endif !------------------------------------------------------------------------------m2
      ...
      endif ! lucas, poroelastic domains
      ...
      if (ELASTIC_SIMULATION) then
        ...
        if(CTD_SEM) then !-------------------------------------------------------m2
        allocate(b_absorb_field_m2(NDIM,NGLLSQUARE,b_num_abs_boundary_faces),stat=ier)
        if (ier /= 0) call exit_MPI_without_rank('error allocating array 2147 for m2')
        if (ier /= 0) stop 'error allocating array b_absorb_field_m2' 
        endif !------------------------------------------------------------------m2
      endif
  ...
  else ! no STACEY_ABSORBING_CONDITIONS
    ! needs dummy array
    b_num_abs_boundary_faces = 0
    if (ELASTIC_SIMULATION) then
      allocate(b_absorb_field(NDIM,NGLLSQUARE,b_num_abs_boundary_faces),stat=ier)
      ...
      if(CTD_SEM) then !--------------------------------------------------------m2
      allocate(b_absorb_field_m2(NDIM,NGLLSQUARE,b_num_abs_boundary_faces),stat=ier)
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 2151 for m2')
      if (ier /= 0) stop 'error allocating array b_absorb_field_m2'
      endif !-------------------------------------------------------------------m2
    endif
