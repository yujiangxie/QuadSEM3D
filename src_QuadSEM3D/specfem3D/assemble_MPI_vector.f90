...
! array to assemble
  ...
  ! send/receive temporary buffers
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: buffer_send_vector !lucas, buffer_send_vector(NDIM,max_nibool_interfaces_ext_mesh,num_interfaces_ext_mesh)
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: buffer_recv_vector
  ! requests
  integer, dimension(:), allocatable :: request_send_vector
  integer, dimension(:), allocatable :: request_recv_vector
...
! setting the value to that of the processor with highest rank
  ...
  if (NPROC > 1) then
    ...
    ! partition border copy into the buffer
    !lucas, to get buffer_send_vector
    do iinterface = 1, num_interfaces_ext_mesh
      do ipoin = 1, nibool_interfaces_ext_mesh(iinterface)
  ...
  do iinterface = 1, num_interfaces_ext_mesh
    if (myrank < my_neighbors_ext_mesh(iinterface)) then  ! lucas, myrank is current rank, and my_neighbors_ext_mesh is the rank for iinterface.
      do ipoin = 1, nibool_interfaces_ext_mesh(iinterface)
        iglob = ibool_interfaces_ext_mesh(ipoin,iinterface)
...
!------------------------------------------------------------------------------------------------------------------------------
!
  !lucas, no conflciting used, that is used one by one for all solvers in each time step,
