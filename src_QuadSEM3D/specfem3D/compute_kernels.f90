...
! and Morency et al. (2009) for poroelastic calculation
  ...
  if (APPROXIMATE_HESS_KL) then
    call compute_kernels_Hessian() ! lucas added for hess_rho_kl, hess_mu_kl, hess_kappa_kl
  endif
...
! see e.g. Tromp et al. (2005)
  ...
  real(kind=CUSTOM_REAL), dimension(5) :: epsilondev_loc_m2,b_epsilondev_loc_m2 ! CTD_SEM
  ...
  if (.not. GPU_MODE) then
     ...
     if (ANISOTROPIC_VELOCITY_KL) then
        !lucas, to compue rho_kl(i,j,k,ielem) and cijkl_kl(21,j,k,ielem)
              ...
              do k = 1, NGLLZ
                 do j = 1, NGLLY
                    do i = 1, NGLLX
                       ...
                       !1.2 lucas, to get cijkl_kl(:,i,j,k,ispec)
                       ! For anisotropic kernels
                       if (ANISOTROPIC_KL) then
                          !lucas, to get prod(21) by epsilondev_loc and b_epsilondev_loc
                       ...
                       !1.3 lucas, to get mu_kl(i,j,k,ispec) and kappa_kl(i,j,k,ispec)
                       else
              ...
              do k = 1, NGLLZ
                 do j = 1, NGLLY
                    do i = 1, NGLLX
                       iglob = ibool(i,j,k,ispec)
                       !1.1 lucas 
                       epsilondev_loc_m2(1) = epsilondev_xx_m2(i,j,k,ispec)
                       epsilondev_loc_m2(2) = epsilondev_yy_m2(i,j,k,ispec)
                       epsilondev_loc_m2(3) = epsilondev_xy_m2(i,j,k,ispec)
                       epsilondev_loc_m2(4) = epsilondev_xz_m2(i,j,k,ispec)
                       epsilondev_loc_m2(5) = epsilondev_yz_m2(i,j,k,ispec)

                       b_epsilondev_loc_m2(1) = b_epsilondev_xx_m2(i,j,k,ispec)
                       b_epsilondev_loc_m2(2) = b_epsilondev_yy_m2(i,j,k,ispec)
                       b_epsilondev_loc_m2(3) = b_epsilondev_xy_m2(i,j,k,ispec)
                       b_epsilondev_loc_m2(4) = b_epsilondev_xz_m2(i,j,k,ispec)
                       b_epsilondev_loc_m2(5) = b_epsilondev_yz_m2(i,j,k,ispec)

                       rho_kl_m2(i,j,k,ispec) =  rho_kl_m2(i,j,k,ispec) &
                            + deltat * dot_product(accel_m2(:,iglob), b_displ_m2(:,iglob))
                       ...
                       if (ANISOTROPIC_KL) then
                          !lucas, to get prod(21) by epsilondev_loc and b_epsilondev_loc
                          call compute_strain_product(prod,epsilon_trace_over_3_m2(i,j,k,ispec),epsilondev_loc_m2, &
                               b_epsilon_trace_over_3_m2(i,j,k,ispec),b_epsilondev_loc_m2) ! lucas, needs to do!!!
                          cijkl_kl(:,i,j,k,ispec) = cijkl_kl(:,i,j,k,ispec) + deltat * prod(:)
                       !1.3 lucas, to get mu_kl(i,j,k,ispec) and kappa_kl(i,j,k,ispec)
                       else
                       ...
                       mu_kl_m2(i,j,k,ispec) =  mu_kl_m2(i,j,k,ispec) &
                       + deltat * (epsilondev_loc_m2(1)*b_epsilondev_loc_m2(1) + epsilondev_loc_m2(2)*b_epsilondev_loc_m2(2) &
                       + (epsilondev_loc_m2(1)+epsilondev_loc_m2(2)) * (b_epsilondev_loc_m2(1)+b_epsilondev_loc_m2(2)) &
                       + 2 * (epsilondev_loc_m2(3)*b_epsilondev_loc_m2(3) + epsilondev_loc_m2(4)*b_epsilondev_loc_m2(4) + &
                       epsilondev_loc_m2(5)*b_epsilondev_loc_m2(5)) )

                          ! kernel for bulk modulus, see e.g. Tromp et al. (2005), equation (18)
                          ! note: multiplication with kappa(x) will be done after the time loop
                          kappa_kl_m2(i,j,k,ispec) = kappa_kl_m2(i,j,k,ispec) &
                               + deltat * (9 * epsilon_trace_over_3_m2(i,j,k,ispec) &
                               * b_epsilon_trace_over_3_m2(i,j,k,ispec))
...
!---------------------------------------------CTD_SEM, Ha, Hb, Hc, Habc--------------------------
  subroutine compute_kernels_el_Ha_Hb_Hc_Habc()
...
! see e.g. Tromp et al. (2005)
  ...
  real(kind=CUSTOM_REAL),dimension(21) :: prod
  real(kind=CUSTOM_REAL), dimension(5) :: epsilondev_loc,b_epsilondev_loc
  real(kind=CUSTOM_REAL), dimension(5) :: epsilondev_loc_m1 ! CTD_SEM 
  real(kind=CUSTOM_REAL), dimension(5) :: epsilondev_loc_m2,b_epsilondev_loc_m2 ! CTD_SEM, only m2 for b_
              ...
              do k = 1, NGLLZ
                 do j = 1, NGLLY
                    do i = 1, NGLLX
                       ...
                       epsilondev_loc_m1(1) = epsilondev_xx_m1(i,j,k,ispec)
                       epsilondev_loc_m1(2) = epsilondev_yy_m1(i,j,k,ispec)
                       epsilondev_loc_m1(3) = epsilondev_xy_m1(i,j,k,ispec)
                       epsilondev_loc_m1(4) = epsilondev_xz_m1(i,j,k,ispec)
                       epsilondev_loc_m1(5) = epsilondev_yz_m1(i,j,k,ispec)
                       endif

                       ! Ha --------------------------------------------ds
                       rho_kl_Ha(i,j,k,ispec) =  rho_kl_Ha(i,j,k,ispec) &
                            + deltat * dot_product(accel(:,iglob), (b_displ_m2(:,iglob)-b_displ(:,iglob))) ! accel* dot ds  
                       !------------------------------------------------
                       ! Hbm --------------------------------------------ds*_m
                       rho_kl_Hbm(i,j,k,ispec) =  rho_kl_Hbm(i,j,k,ispec) &
                            + deltat * dot_product((accel_m2(:,iglob)-accel(:,iglob)), b_displ(:,iglob)) ! d_m accel* dot s
                       !------------------------------------------------
                       ! Hbs --------------------------------------------ds*_s
                       if(compute_approx_Hessian) then 
                       rho_kl_Hbs(i,j,k,ispec) =  rho_kl_Hbs(i,j,k,ispec) &
                            + deltat * dot_product((accel_m1(:,iglob)-accel(:,iglob)), b_displ(:,iglob)) ! d_s accel* dot s
                       endif
                       !------------------------------------------------

                       ! Hc -------------------------------------------- Hc =0 when model = (rho, kappa, mu)
                       rho_kl_Hc(i,j,k,ispec) =  0._CUSTOM_REAL
                       !------------------------------------------------
                       ! Habc--------------------------------------------
                       ...
                       !1.3 lucas, to get mu_kl(i,j,k,ispec) and kappa_kl(i,j,k,ispec)
                          ...
                          ! note: multiplication with 2*mu(x) will be done after the time loop
                          ! Ha------------------------------------------------------------------ds
                          mu_kl_Ha(i,j,k,ispec) =  mu_kl_Ha(i,j,k,ispec) &
                           + deltat * &
                             (&
                             epsilondev_loc(1)*(b_epsilondev_loc_m2(1)-b_epsilondev_loc(1)) & 
                           + epsilondev_loc(2)*(b_epsilondev_loc_m2(2)-b_epsilondev_loc(2)) &
                           + (epsilondev_loc(1)+epsilondev_loc(2))* & 
                           ((b_epsilondev_loc_m2(1)-b_epsilondev_loc(1))+(b_epsilondev_loc_m2(2)-b_epsilondev_loc(2))) &
                           + 2 * (epsilondev_loc(3)*(b_epsilondev_loc_m2(3)-b_epsilondev_loc(3)) + & 
                                  epsilondev_loc(4)*(b_epsilondev_loc_m2(4)-b_epsilondev_loc(4)) + &
                                  epsilondev_loc(5)*(b_epsilondev_loc_m2(5)-b_epsilondev_loc(5))) &
                             )

                          ! kernel for bulk modulus, see e.g. Tromp et al. (2005), equation (18)
                          ! note: multiplication with kappa(x) will be done after the time loop
                          kappa_kl_Ha(i,j,k,ispec) = kappa_kl_Ha(i,j,k,ispec) &
                               + deltat * &
                                 (9 * epsilon_trace_over_3(i,j,k,ispec) &
                               * (b_epsilon_trace_over_3_m2(i,j,k,ispec)-b_epsilon_trace_over_3(i,j,k,ispec)) &
                                 )
                          !-----------------------------------------------------------------------
                          ! Hbm------------------------------------------------------------------ds*_m
                          mu_kl_Hbm(i,j,k,ispec) =  mu_kl_Hbm(i,j,k,ispec) &
                               + deltat * &
                               ( &
                               (epsilondev_loc_m2(1)-epsilondev_loc(1))*b_epsilondev_loc(1) + & 
                               (epsilondev_loc_m2(2)-epsilondev_loc(2))*b_epsilondev_loc(2) + &
                               ((epsilondev_loc_m2(1)-epsilondev_loc(1))+(epsilondev_loc_m2(2)-epsilondev_loc(2))) * & 
                                (b_epsilondev_loc(1)+b_epsilondev_loc(2)) + &
                               2 * ((epsilondev_loc_m2(3)-epsilondev_loc(3))*b_epsilondev_loc(3) + & 
                                    (epsilondev_loc_m2(4)-epsilondev_loc(4))*b_epsilondev_loc(4) + &
                                    (epsilondev_loc_m2(5)-epsilondev_loc(5))*b_epsilondev_loc(5)) &
                                )

                          ! kernel for bulk modulus, see e.g. Tromp et al. (2005), equation (18)
                          ! note: multiplication with kappa(x) will be done after the time loop
                          kappa_kl_Hbm(i,j,k,ispec) = kappa_kl_Hbm(i,j,k,ispec) &
                               + deltat * &
                                 (9 * (epsilon_trace_over_3_m2(i,j,k,ispec)-epsilon_trace_over_3(i,j,k,ispec)) &
                               * b_epsilon_trace_over_3(i,j,k,ispec) &
                                 )
                          !-----------------------------------------------------------------------
                          ! Hbs------------------------------------------------------------------ds*_s
                          if(compute_approx_Hessian) then 
                          mu_kl_Hbs(i,j,k,ispec) =  mu_kl_Hbs(i,j,k,ispec) &
                               + deltat * &
                               ( &
                               (epsilondev_loc_m1(1)-epsilondev_loc(1))*b_epsilondev_loc(1) + & 
                               (epsilondev_loc_m1(2)-epsilondev_loc(2))*b_epsilondev_loc(2) + &
                               ((epsilondev_loc_m1(1)-epsilondev_loc(1))+(epsilondev_loc_m1(2)-epsilondev_loc(2))) * & 
                                (b_epsilondev_loc(1)+b_epsilondev_loc(2)) + &
                               2 * ((epsilondev_loc_m1(3)-epsilondev_loc(3))*b_epsilondev_loc(3) + & 
                                    (epsilondev_loc_m1(4)-epsilondev_loc(4))*b_epsilondev_loc(4) + &
                                    (epsilondev_loc_m1(5)-epsilondev_loc(5))*b_epsilondev_loc(5)) &
                                )

                          ! kernel for bulk modulus, see e.g. Tromp et al. (2005), equation (18)
                          ! note: multiplication with kappa(x) will be done after the time loop
                          kappa_kl_Hbs(i,j,k,ispec) = kappa_kl_Hbs(i,j,k,ispec) &
                               + deltat * & 
                                 (9 * (epsilon_trace_over_3_m1(i,j,k,ispec)-epsilon_trace_over_3(i,j,k,ispec)) &
                               * b_epsilon_trace_over_3(i,j,k,ispec) &
                                 )
                          endif
                          !-----------------------------------------------------------------------
                          ! Hc------------------------------------------------------------------
                          mu_kl_Hc(i,j,k,ispec) =  0._CUSTOM_REAL

                          ! kernel for bulk modulus, see e.g. Tromp et al. (2005), equation (18)
                          ! note: multiplication with kappa(x) will be done after the time loop
                          kappa_kl_Hc(i,j,k,ispec) = 0._CUSTOM_REAL
                          !-----------------------------------------------------------------------
                   
                          ! Habc-----------------------------------------------------------lucas, these Habc here without multipy rho, mu, kappa, not used
...
!-------------------------------------------------------------------------------------------------
!
  !lucas, to get rho_ac_kl(i,j,k,ispec), and kappa_ac_kl(i,j,k,ispec)
...
! see e.g. Tromp et al. (2005)
  ...
  do ispec = 1, NSPEC_AB
    ...
    if (ispec_is_acoustic(ispec)) then
      ...
      !lucas, to get accel_elm
      call compute_gradient_in_acoustic(ispec,potential_acoustic,accel_elm)
        !!!!===========interporation for above function:compute_gradient_in_acoustic====================
        !lucas, out: vector_field_element(NDIM,NGLLX,NGLLY,NGLLZ)=(temp1(i,j,k)*xizl + temp2(i,j,k)*etazl + temp3(i,j,k)*gammazl) * rho_invl, 
        !lucas, where temp1 or temp2 or temp3 = f(scalar_field).
        !lucas, where compute_gradient_in_acoustic(ispec,scalar_field,vector_field_element)
        !!!!===========interporation for above function:compute_gradient_in_acoustic====================
...
!-------------------------------------------------------------------------------------------------
!
  !lucas, to get acoustic: hess_ac_kl,hess_rho_ac_kl, hess_kappa_ac_kl,    
  !lucas         elastic:  hess_kl,   hess_rho_kl,    hess_mu_kl,   hess_kappa_kl 
  ...
  do ispec = 1, NSPEC_AB
   ...
   ! adjoint fields: acceleration vector
      call compute_gradient_in_acoustic(ispec,potential_dot_dot_acoustic,accel_elm)!lucas to get accel_elm

      ! adjoint fields: acceleration vector
      call compute_gradient_in_acoustic(ispec,b_potential_dot_dot_acoustic,b_accel_elm)!lucas, to get b_accel_elm

      ! backward fields: displacement vector
      call compute_gradient_in_acoustic(ispec,b_potential_dot_acoustic,b_veloc_elm)! lucas, to get b_veloc_elm
