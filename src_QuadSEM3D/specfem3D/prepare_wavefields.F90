...
! initializes arrays
  ...
  if (ELASTIC_SIMULATION) then
    ...
    if(CTD_SEM) then !---------------------------------m2
    displ_m2(:,:) = 0._CUSTOM_REAL
    veloc_m2(:,:) = 0._CUSTOM_REAL
    accel_m2(:,:) = 0._CUSTOM_REAL
    ! put negligible initial value to avoid very slow underflow trapping
    if (FIX_UNDERFLOW_PROBLEM) displ_m2(:,:) = VERYSMALLVAL
     if(compute_approx_Hessian) then !-------m1
     displ_m1(:,:) = 0._CUSTOM_REAL
     veloc_m1(:,:) = 0._CUSTOM_REAL
     accel_m1(:,:) = 0._CUSTOM_REAL
     ! put negligible initial value to avoid very slow underflow trapping
     if (FIX_UNDERFLOW_PROBLEM) displ_m1(:,:) = VERYSMALLVAL
     endif !---------------------------------m1
    endif !--------------------------------------------m2
