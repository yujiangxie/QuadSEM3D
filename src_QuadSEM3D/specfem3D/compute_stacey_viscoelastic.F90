...
!-------------------------------------------------------------- m2 --------------------------------------
! CTD_SEM
  subroutine compute_stacey_viscoelastic_m2(NSPEC_AB,NGLOB_AB,accel_m2, & !CTD_SEM
                        ibool,iphase, &
                        abs_boundary_normal,abs_boundary_jacobian2Dw, &
                        abs_boundary_ijk,abs_boundary_ispec, &
                        num_abs_boundary_faces,veloc_m2,rho_vp_m2,rho_vs_m2, & !CTD_SEM
                        ispec_is_elastic,SIMULATION_TYPE,SAVE_FORWARD, &
                        it,b_num_abs_boundary_faces,b_reclen_field,b_absorb_field_m2) !CTD_SEM, lucas b_reclen_field is common parameter

  use constants

  use specfem_par, only: it_dsm, it_fk, Veloc_dsm_boundary, Tract_dsm_boundary, Veloc_axisem, Tract_axisem, Tract_axisem_time

  use specfem_par_elastic, only: displ_m2 ! CTD_SEM
...
! acceleration
  real(kind=CUSTOM_REAL), dimension(NDIM,NGLOB_AB) :: accel_m2 ! CTD_SEM
  integer, dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: ibool
...
! Stacey conditions
  real(kind=CUSTOM_REAL), dimension(NDIM,NGLOB_AB) :: veloc_m2 ! CTD_SEM
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: rho_vp_m2,rho_vs_m2 !CTD_SEM
...
! adjoint simulations
  ...
  integer:: it
  integer:: b_num_abs_boundary_faces,b_reclen_field
  real(kind=CUSTOM_REAL),dimension(NDIM,NGLLSQUARE,b_num_abs_boundary_faces):: b_absorb_field_m2 !CTD_SEM

  logical:: SAVE_FORWARD

! local parameters
  real(kind=CUSTOM_REAL) vx,vy,vz,nx,ny,nz,tx,ty,tz,jacobianw ! lucas, vn removed
  real(kind=CUSTOM_REAL) vx_m2,vy_m2,vz_m2,vn_m2,tx_m2,ty_m2,tz_m2 !CTD_SEM
  integer :: ispec,iglob,i,j,k,iface,igll
  ...
  do iface = 1,num_abs_boundary_faces
    ...
    if (ispec_is_elastic(ispec)) then
      ...
      do igll = 1,NGLLSQUARE
        ...
        vx_m2 = veloc_m2(1,iglob)
        vy_m2 = veloc_m2(2,iglob)
        vz_m2 = veloc_m2(3,iglob)
...
! *********************************************************************************
        ...
        ! velocity component in normal direction (normal points out of element)
        vn_m2 = vx_m2*nx + vy_m2*ny + vz_m2*nz

        ! stacey term: velocity vector component * vp * rho in normal direction + vs * rho component tangential to it
        tx_m2 = rho_vp_m2(i,j,k,ispec)*vn_m2*nx + rho_vs_m2(i,j,k,ispec)*(vx_m2-vn_m2*nx)
        ty_m2 = rho_vp_m2(i,j,k,ispec)*vn_m2*ny + rho_vs_m2(i,j,k,ispec)*(vy_m2-vn_m2*ny)
        tz_m2 = rho_vp_m2(i,j,k,ispec)*vn_m2*nz + rho_vs_m2(i,j,k,ispec)*(vz_m2-vn_m2*nz)
...
! *********************************************************************************
        ...
        ! adds stacey term (weak form)
        accel_m2(1,iglob) = accel_m2(1,iglob) - tx_m2*jacobianw
        accel_m2(2,iglob) = accel_m2(2,iglob) - ty_m2*jacobianw
        accel_m2(3,iglob) = accel_m2(3,iglob) - tz_m2*jacobianw

        ! adjoint simulations
        if (SIMULATION_TYPE == 1 .and. SAVE_FORWARD) then
          b_absorb_field_m2(1,igll,iface) = tx_m2*jacobianw
          b_absorb_field_m2(2,igll,iface) = ty_m2*jacobianw
          b_absorb_field_m2(3,igll,iface) = tz_m2*jacobianw
        endif !adjoint

          !! CD CD added this
          if (SAVE_RUN_BOUN_FOR_KH_INTEGRAL) then
              write(237) b_absorb_field_m2(1,igll,iface), b_absorb_field_m2(2,igll,iface), b_absorb_field_m2(3,igll,iface)
              write(238) displ_m2(1,iglob), displ_m2(2,iglob), displ_m2(3,iglob)
          endif
  ...
  if (SIMULATION_TYPE == 1 .and. SAVE_FORWARD) then
    ! writes out absorbing boundary value
    call write_abs(IOABS_m2,b_absorb_field_m2,b_reclen_field,it) !CTD_SEM, see prepare_timerun for IOABS_m2
  endif
  ...
  end subroutine compute_stacey_viscoelastic_m2



!-------------------------------------------------------------- m1 -------------------------------------- approx Hessian
! CTD_SEM
  subroutine compute_stacey_viscoelastic_m1(NSPEC_AB,NGLOB_AB,accel_m1, & !CTD_SEM
                        ibool,iphase, &
                        abs_boundary_normal,abs_boundary_jacobian2Dw, &
                        abs_boundary_ijk,abs_boundary_ispec, &
                        num_abs_boundary_faces,veloc_m1,rho_vp,rho_vs, & !CTD_SEM
                        ispec_is_elastic,SIMULATION_TYPE,SAVE_FORWARD, &
                        it,b_num_abs_boundary_faces,b_reclen_field,b_absorb_field_m1) !CTD_SEM, lucas b_reclen_field is common parameter

  use constants

  use specfem_par, only: it_dsm, it_fk, Veloc_dsm_boundary, Tract_dsm_boundary, Veloc_axisem, Tract_axisem, Tract_axisem_time

  use specfem_par_elastic, only: displ_m1 ! CTD_SEM
...
! acceleration
  real(kind=CUSTOM_REAL), dimension(NDIM,NGLOB_AB) :: accel_m1 ! CTD_SEM
  integer, dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: ibool
...
! Stacey conditions
  real(kind=CUSTOM_REAL), dimension(NDIM,NGLOB_AB) :: veloc_m1 ! CTD_SEM
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ,NSPEC_AB) :: rho_vp,rho_vs
...
! adjoint simulations
  ...
  integer:: it
  integer:: b_num_abs_boundary_faces,b_reclen_field
  real(kind=CUSTOM_REAL),dimension(NDIM,NGLLSQUARE,b_num_abs_boundary_faces):: b_absorb_field_m1 !CTD_SEM

  logical:: SAVE_FORWARD

! local parameters
  real(kind=CUSTOM_REAL) vx,vy,vz,nx,ny,nz,tx,ty,tz,jacobianw !lucas, vn removed
  real(kind=CUSTOM_REAL) vx_m1,vy_m1,vz_m1,vn_m1,tx_m1,ty_m1,tz_m1 !CTD_SEM
  integer :: ispec,iglob,i,j,k,iface,igll
  ...
  do iface = 1,num_abs_boundary_faces
    ...
    if (ispec_is_elastic(ispec)) then
      ...
      do igll = 1,NGLLSQUARE
        ...
        vx_m1 = veloc_m1(1,iglob)
        vy_m1 = veloc_m1(2,iglob)
        vz_m1 = veloc_m1(3,iglob)
...
! *********************************************************************************
        ...
        ! velocity component in normal direction (normal points out of element)
        vn_m1 = vx_m1*nx + vy_m1*ny + vz_m1*nz

        ! stacey term: velocity vector component * vp * rho in normal direction + vs * rho component tangential to it
        tx_m1 = rho_vp(i,j,k,ispec)*vn_m1*nx + rho_vs(i,j,k,ispec)*(vx_m1-vn_m1*nx)
        ty_m1 = rho_vp(i,j,k,ispec)*vn_m1*ny + rho_vs(i,j,k,ispec)*(vy_m1-vn_m1*ny)
        tz_m1 = rho_vp(i,j,k,ispec)*vn_m1*nz + rho_vs(i,j,k,ispec)*(vz_m1-vn_m1*nz)
...
! *********************************************************************************
        ...
        ! adds stacey term (weak form)
        accel_m1(1,iglob) = accel_m1(1,iglob) - tx_m1*jacobianw
        accel_m1(2,iglob) = accel_m1(2,iglob) - ty_m1*jacobianw
        accel_m1(3,iglob) = accel_m1(3,iglob) - tz_m1*jacobianw

        ! adjoint simulations
        if (SIMULATION_TYPE == 1 .and. SAVE_FORWARD) then
          b_absorb_field_m1(1,igll,iface) = tx_m1*jacobianw
          b_absorb_field_m1(2,igll,iface) = ty_m1*jacobianw
          b_absorb_field_m1(3,igll,iface) = tz_m1*jacobianw
        endif !adjoint

          !! CD CD added this
          if (SAVE_RUN_BOUN_FOR_KH_INTEGRAL) then
              write(237) b_absorb_field_m1(1,igll,iface), b_absorb_field_m1(2,igll,iface), b_absorb_field_m1(3,igll,iface)
              write(238) displ_m1(1,iglob), displ_m1(2,iglob), displ_m1(3,iglob)
          endif
  ...
  !lucas 4, we do not need to store when used this for adjoint simulation
  ! adjoint simulations: stores absorbed wavefield part
!  if (SIMULATION_TYPE == 1 .and. SAVE_FORWARD) then
!    ! writes out absorbing boundary value
!    call write_abs(IOABS+1000,b_absorb_field_m1,b_reclen_field,it) !CTD_SEM, see prepare_timerun for the 1000
!  endif
  ...
  end subroutine compute_stacey_viscoelastic_m1
...
! for elastic solver
!lucas, b_accel(:,iglob) = b_accel(:,iglob) - b_absorb_field(:,igll,iface), where b_absorb_field(:,igll,iface) is obtained in compute_stacey_viscoelastic above.
! absorbing boundary term for elastic media (Stacey conditions)
...
!------------------------------------------------------- m2 -----------------------------------------
  subroutine compute_stacey_viscoelastic_backward_m2(NSPEC_AB, &
                        ibool,iphase, &
                        abs_boundary_ijk,abs_boundary_ispec, &
                        num_abs_boundary_faces, &
                        ispec_is_elastic,SIMULATION_TYPE, &
                        NSTEP,it,NGLOB_ADJOINT,b_accel_m2, & !CTD_SEM
                        b_num_abs_boundary_faces,b_reclen_field,b_absorb_field_m2) !lucas, CTD_SEM, b_reclen_field is common parameter
...
! adjoint simulations
  ...
  real(kind=CUSTOM_REAL),dimension(NDIM,NGLOB_ADJOINT):: b_accel_m2 !CTD_SEM
...
! local parameters
  ...
  ! checks
  if (SIMULATION_TYPE /= 3) &
    call exit_MPI(myrank,'error calling routine compute_stacey_viscoelastic_backward() for m2 with wrong SIMULATION_TYPE')
  ...
  ! reads in absorbing boundary array (when first phase is running)
  ! note: the index NSTEP-it+1 is valid if b_displ is read in after the Newmark scheme
  call read_abs(IOABS_m2,b_absorb_field_m2,b_reclen_field,NSTEP-it+1) !CTD_SEM, see prepare_timerun for the IOABS_m2
  ...
  do iface = 1,num_abs_boundary_faces
    ...
    if (ispec_is_elastic(ispec)) then
      ...
      do igll = 1,NGLLSQUARE
        ...
        ! adjoint simulations
        b_accel_m2(:,iglob) = b_accel_m2(:,iglob) - b_absorb_field_m2(:,igll,iface) !CTD_SEM
      enddo
    endif ! ispec_is_elastic
  enddo

  end subroutine compute_stacey_viscoelastic_backward_m2
