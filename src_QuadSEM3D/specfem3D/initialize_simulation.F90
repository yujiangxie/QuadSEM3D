...
! read the value of NSPEC_AB and NGLOB_AB because we need it to define some array sizes below
  ...
  if(CTD_SEM) then !---------------------m2, where allocate(rhostore) is done in read_mesh_databases.bin
  ! material properties
  allocate(kappastore_m2(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 2413 for m2')
  allocate(mustore_m2(NGLLX,NGLLY,NGLLZ,NSPEC_AB),stat=ier)
  if (ier /= 0) call exit_MPI_without_rank('error allocating array 2414 for m2')
  if (ier /= 0) stop 'error allocating arrays for material properties for m2'
  endif !--------------------------------m2
