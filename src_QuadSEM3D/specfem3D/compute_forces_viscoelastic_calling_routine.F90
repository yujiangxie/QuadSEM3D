        ...
        if (num_coupling_ac_el_faces > 0) then
          if (SIMULATION_TYPE == 1) then
            ...
            call compute_coupling_viscoelastic_ac(NSPEC_AB,NGLOB_AB, &
                                ...
                                PML_CONDITIONS, &
                                SIMULATION_TYPE,.false., &
                                potential_acoustic,potential_dot_acoustic)!lucas, inout:accel, the others are input.
...
!--------------------------------------------------------------------m2 -----------------
subroutine compute_forces_viscoelastic_calling_m2() ! CTD_SEM
  ...
  ! Do this only for dynamic rupture simulations
  if (SIMULATION_TYPE_DYN) then
    call synchronize_MPI_vector_blocking_ord(NPROC,NGLOB_AB,displ_m2, &
                                     num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                                     nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                                     my_neighbors_ext_mesh,myrank)
    call synchronize_MPI_vector_blocking_ord(NPROC,NGLOB_AB,veloc_m2, &
                                     num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                                     nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                                     my_neighbors_ext_mesh,myrank)
  endif
...
!                        R_xx_lddrk,R_yy_lddrk,R_xy_lddrk,R_xz_lddrk,R_yz_lddrk, & 
                        ...
                        .false.)
!    print *,'compute_forces_viscoelastic_m2: finished'

    ! computes additional contributions
    if (iphase == 1) then
      ! adds elastic absorbing boundary term to acceleration (Stacey conditions)
      if (STACEY_ABSORBING_CONDITIONS) then
        call compute_stacey_viscoelastic_m2(NSPEC_AB,NGLOB_AB,accel_m2, & ! CTD_SEM, done! 
                         ibool,iphase, &
                         abs_boundary_normal,abs_boundary_jacobian2Dw, &
                         abs_boundary_ijk,abs_boundary_ispec, &
                         num_abs_boundary_faces, &
                         veloc_m2,rho_vp_m2,rho_vs_m2, & !CTD_SEM
                         ispec_is_elastic,SIMULATION_TYPE,SAVE_FORWARD, &
                         it, &
                         b_num_abs_boundary_faces,b_reclen_field,b_absorb_field_m2) ! CTD_SEM
      endif


!      print *,'compute_stacey_viscoelastic_m2: finished'
      ! acoustic coupling
      if (ACOUSTIC_SIMULATION) then
        if (num_coupling_ac_el_faces > 0) then
          if (SIMULATION_TYPE == 1) then
      ...
      !
      ! adds source term (single-force/moment-tensor solution)
      call compute_add_sources_viscoelastic_m2() ! CTD_SEM
!      print *,'compute_add_sources_viscoelastic_m2(): finished'
    ...
    if (iphase == 1) then
       ! sends accel values to corresponding MPI interface neighbors
       call assemble_MPI_vector_async_send(NPROC,NGLOB_AB,accel_m2, &  ! lucas, only needs to change here since the other no conflicting usage
               buffer_send_vector_ext_mesh,buffer_recv_vector_ext_mesh, &
               num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
               nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
               my_neighbors_ext_mesh, &
      ...
      ! waits for send/receive requests to be completed and assembles values
      call assemble_MPI_vector_async_w_ord(NPROC,NGLOB_AB,accel_m2, & ! lucas, 
                            buffer_recv_vector_ext_mesh,num_interfaces_ext_mesh, &
                            max_nibool_interfaces_ext_mesh, &
                            nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                            request_send_vector_ext_mesh,request_recv_vector_ext_mesh, &
...
!        which at this point are stored in the array 'accel'
 ...
 ! multiplies with inverse of mass matrix (note: rmass has been inverted already)
 ! CTD_SEM
  accel_m2(1,:) = accel_m2(1,:)*rmassx_m2(:)
  accel_m2(2,:) = accel_m2(2,:)*rmassy_m2(:)
  accel_m2(3,:) = accel_m2(3,:)*rmassz_m2(:)
...
!   updates the velocity term which requires a(t+delta)
  if (USE_LDDRK) then
    ...
    veloc_m2(:,:) = veloc_m2(:,:) + deltatover2*accel_m2(:,:) ! CTD_SEM
  endif
...
end subroutine compute_forces_viscoelastic_calling_m2

!--------------------------------------------------------------------m1 ----------------- for approx Hessian
subroutine compute_forces_viscoelastic_calling_m1() ! CTD_SEM
...
! elastic term
    ! CTD_SEM, done!
    call compute_forces_viscoelastic_m1(iphase, &
                        displ_m1,veloc_m1,accel_m1, &
                        alphaval,betaval,gammaval, & ! lucas,  alphaval,betaval,gammaval only depend on vpmax_glob
                        R_trace_m1,R_xx_m1,R_yy_m1,R_xy_m1,R_xz_m1,R_yz_m1, &
!                        R_trace_lddrk, & ! lucas, lddrk not considered right now, but can be considered
!                        R_xx_lddrk,R_yy_lddrk,R_xy_lddrk,R_xz_lddrk,R_yz_lddrk, & ! lucas, not consiered for m1 right now
                        epsilondev_trace_m1,epsilondev_xx_m1,epsilondev_yy_m1,epsilondev_xy_m1, &
                        epsilondev_xz_m1,epsilondev_yz_m1,epsilon_trace_over_3_m1, & !lucas, epsilon_trace_over_3_m1 is wavefield related
                        .false.)
    ...
    if (iphase == 1) then
      ! adds elastic absorbing boundary term to acceleration (Stacey conditions)
      if (STACEY_ABSORBING_CONDITIONS) then
        call compute_stacey_viscoelastic_m1(NSPEC_AB,NGLOB_AB,accel_m1, & ! CTD_SEM, done! 
                         ibool,iphase, &
                         abs_boundary_normal,abs_boundary_jacobian2Dw, &
                         abs_boundary_ijk,abs_boundary_ispec, &
                         num_abs_boundary_faces, &
                         veloc,rho_vp,rho_vs, & !lucas, the same 
                         ispec_is_elastic,SIMULATION_TYPE,SAVE_FORWARD, &
                         it, &
                         b_num_abs_boundary_faces,b_reclen_field,b_absorb_field_m1) ! CTD_SEM
      endif
      ...
      !
      ! adds source term (single-force/moment-tensor solution)
      call compute_add_sources_viscoelastic_m1() ! CTD_SEM
    ...
    if (iphase == 1) then
       ! sends accel values to corresponding MPI interface neighbors
       call assemble_MPI_vector_async_send(NPROC,NGLOB_AB,accel_m1, &  ! lucas, only needs to change here since the other no conflicting usage
               buffer_send_vector_ext_mesh,buffer_recv_vector_ext_mesh, &
               num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
               nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
               my_neighbors_ext_mesh, &
      ...
      ! waits for send/receive requests to be completed and assembles values
      call assemble_MPI_vector_async_w_ord(NPROC,NGLOB_AB,accel_m1, & ! lucas, 
                            buffer_recv_vector_ext_mesh,num_interfaces_ext_mesh, &
                            max_nibool_interfaces_ext_mesh, &
                            nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                            request_send_vector_ext_mesh,request_recv_vector_ext_mesh, &
...
!        which at this point are stored in the array 'accel'
 ...
 ! multiplies with inverse of mass matrix (note: rmass has been inverted already)
 ! CTD_SEM
  accel_m1(1,:) = accel_m1(1,:)*rmassx_m1(:)
  accel_m1(2,:) = accel_m1(2,:)*rmassy_m1(:)
  accel_m1(3,:) = accel_m1(3,:)*rmassz_m1(:)
...
!   updates the velocity term which requires a(t+delta)
  if (USE_LDDRK) then
    ...
    veloc_m1(:,:) = veloc_m1(:,:) + deltatover2*accel_m1(:,:) ! CTD_SEM
  endif
...
end subroutine compute_forces_viscoelastic_calling_m1
...
!----------------------------------------------------------------------m2 --------------------------------------
subroutine compute_forces_viscoelastic_backward_calling_m2()
  ...
  ! checks
  if (SIMULATION_TYPE /= 3) &
    call exit_MPI(myrank,'error calling compute_forces_viscoelastic_backward() for m2 with wrong SIMULATION_TYPE')
  ! Lucas A
  ! distinguishes two runs: for elements in contact with MPI interfaces, and elements within the partitions
  do iphase = 1,2
...
! elastic term
    ! adjoint simulations: backward/reconstructed wavefield
    !lucas 1, CTD_SEM, done!
    call compute_forces_viscoelastic_m2(iphase, &
                        b_displ_m2,b_veloc_m2,b_accel_m2, &
                        b_alphaval_m2,b_betaval_m2,b_gammaval_m2, &
                        b_R_trace_m2,b_R_xx_m2,b_R_yy_m2,b_R_xy_m2,b_R_xz_m2,b_R_yz_m2, &
!                        b_R_trace_lddrk, & ! lucas, not used right now
!                        b_R_xx_lddrk,b_R_yy_lddrk,b_R_xy_lddrk,b_R_xz_lddrk,b_R_yz_lddrk, & ! lucas, not used right now
                        b_epsilondev_trace_m2,b_epsilondev_xx_m2,b_epsilondev_yy_m2,b_epsilondev_xy_m2, &
                        b_epsilondev_xz_m2,b_epsilondev_yz_m2,b_epsilon_trace_over_3_m2, &
                        .true.)
    ! lucas 2
    ! computes additional contributions
    if (iphase == 1) then
      ! adds elastic absorbing boundary term to acceleration (Stacey conditions)
      if (STACEY_ABSORBING_CONDITIONS) then
        ! lucas 2.1, CTD_SEM, done!
        call compute_stacey_viscoelastic_backward_m2(NSPEC_AB, &
                         ibool,iphase, &
                         abs_boundary_ijk,abs_boundary_ispec, &
                         num_abs_boundary_faces, &
                         ispec_is_elastic,SIMULATION_TYPE, &
      ...
      ! note: we will add all source contributions in the first pass, when iphase == 1
      !       to avoid calling the same routine twice and to check if the source element is an inner/outer element
      ! lucas, 2.4 CTD_SEM, done!
      call compute_add_sources_viscoelastic_backward_m2()
    ...
    if (iphase == 1) then
      ! sends accel values to corresponding MPI interface neighbors
      ! adjoint simulations
      call assemble_MPI_vector_async_send(NPROC,NGLOB_ADJOINT,b_accel_m2, & ! lucas, only need here
                 b_buffer_send_vector_ext_mesh,b_buffer_recv_vector_ext_mesh, &
                 num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                 nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                 my_neighbors_ext_mesh, &
      ...
      ! waits for send/receive requests to be completed and assembles values
      ! adjoint simulations
      call assemble_MPI_vector_async_w_ord(NPROC,NGLOB_ADJOINT,b_accel_m2, & ! lucas, only need here
                             b_buffer_recv_vector_ext_mesh,num_interfaces_ext_mesh, &
                             max_nibool_interfaces_ext_mesh, &
                             nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                             b_request_send_vector_ext_mesh,b_request_recv_vector_ext_mesh, &
  ...
  ! multiplies with inverse of mass matrix (note: rmass has been inverted already)
  ! adjoint simulations
  b_accel_m2(1,:) = b_accel_m2(1,:)*rmassx_m2(:)
  b_accel_m2(2,:) = b_accel_m2(2,:)*rmassy_m2(:)
  b_accel_m2(3,:) = b_accel_m2(3,:)*rmassz_m2(:)
...
!   updates the velocity term which requires a(t+delta)
  ! adjoint simulations
 ! lucas D, CTD_SEM
  b_veloc_m2(:,:) = b_veloc_m2(:,:) + b_deltatover2*b_accel_m2(:,:)

end subroutine compute_forces_viscoelastic_backward_calling_m2
