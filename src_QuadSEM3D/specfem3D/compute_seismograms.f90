...
!=====================================================================
  ...
  double precision :: dxd_m2,dyd_m2,dzd_m2,vxd_m2,vyd_m2,vzd_m2,axd_m2,ayd_m2,azd_m2 !CTD_SEM
  ...
  do irec_local = 1,nrec_local
    ...
    !1.lucas, to get dxd,dyd,dzd,vxd,vyd,vzd,axd,ayd,azd,pd.
    ...
    case (1,2)
      ...
      if (ispec_is_elastic(ispec)) then
        ...
        !lucas, ! returns displacement/velocity/acceleration (dxd,..,vxd,..,axd,.. ) at receiver location, e.g., dxd = dxd + dble(displ(1,iglob))*hlagrange, where hlagrange = hxir(i) * hetar(j) * hgammar(k).
        call compute_interpolated_dva_viscoelast(displ,veloc,accel,NGLOB_AB, &
                                      ispec,NSPEC_AB,ibool, &
                                      hxir,hetar,hgammar, &
                                      dxd,dyd,dzd,vxd,vyd,vzd,axd,ayd,azd)
        if(CTD_SEM) then !-----m2
        call compute_interpolated_dva_viscoelast(displ_m2,veloc_m2,accel_m2,NGLOB_AB, &
                                      ispec,NSPEC_AB,ibool, &
                                      hxir,hetar,hgammar, &
                                      dxd_m2,dyd_m2,dzd_m2,vxd_m2,vyd_m2,vzd_m2,axd_m2,ayd_m2,azd_m2)
        endif !----------------m2
      endif ! elastic--------------------------------------------------------
      ...
      if (ispec_is_acoustic(ispec)) then
        ! displacement vector
        !lucas, out: vector_field_element(NDIM,NGLLX,NGLLY,NGLLZ)(or displ_element)=(temp1(i,j,k)*xizl + temp2(i,j,k)*etazl + temp3(i,j,k)*gammazl) * rho_invl, 
        !lucas, where temp1 or temp2 or temp3 = f[scalar_field(or b_potential_acoustic)].
        !lucas, i.e., to get displ_element (NDIM,NGLLX,NGLLY,NGLLZ).
        call compute_gradient_in_acoustic(ispec,potential_acoustic,displ_element)

        ! velocity vector
        !lucas, i.e., to get veloc_element (NDIM,NGLLX,NGLLY,NGLLZ)
        call compute_gradient_in_acoustic(ispec,potential_dot_acoustic,veloc_element)

        ! acceleration vector
        !lucas, i.e., to get accel_element (NDIM,NGLLX,NGLLY,NGLLZ)
        ...
        !lucas,out: dxd,dyd,dzd,   vxd,vyd,vzd,   axd,ayd,azd,   pd.  e.g.,dxd = dxd + hlagrange*displ_element(1,i,j,k).
    ...
    case (3)
      ...
      if (ispec_is_elastic(ispec)) then
        ...
        call compute_interpolated_dva_viscoelast(b_displ,b_veloc,b_accel,NGLOB_ADJOINT, &
                                      ...
                                      dxd,dyd,dzd,vxd,vyd,vzd,axd,ayd,azd)
        if(CTD_SEM) then !-----------m2
        call compute_interpolated_dva_viscoelast(b_displ_m2,b_veloc_m2,b_accel_m2,NGLOB_ADJOINT, &
                                      ispec,NSPEC_AB,ibool, &
                                      hxir,hetar,hgammar, &
                                      dxd_m2,dyd_m2,dzd_m2,vxd_m2,vyd_m2,vzd_m2,axd_m2,ayd_m2,azd_m2)
        endif !----------------------m2
    ...
    !2.lucas, to get Mxx_der,Myy_der,Mzz_der,Mxy_der,Mxz_der,Myz_der, sloc_der, i.e., ! adjoint source frechet derivatives
    ...
    if (SIMULATION_TYPE == 2) then
      ...
      if (ispec_is_elastic(ispec)) then
        ...
        !lucas, out: eps_s(NDIM,NDIM), eps_m_s(NDIM)
        !lucas, in: ispec,displ_element,Mxx,Myy,Mzz,Mxy,Mxz,Myz
        ...
        stf = comp_source_time_function(dble(NSTEP-it)*DT-t0-tshift_src(irec),hdur_Gaussian(irec))!lucas, stf=amp of it.
    ...
    endif !lucas SIMULATION_TYPE == 2
    
    !3.lucas
    if (SIMULATION_TYPE == 2) then
      ! adjoint simulations
      ! adjoint "receiver" N/E/Z orientations given by nu_source array
      rotation_seismo(:,:) = nu_source(:,:,irec) !lucas, commom parameter
    else
      rotation_seismo(:,:) = nu(:,:,irec)!lucas, commom parameter
    endif

    !4.lucas, to get seismograms_d, seismograms_v, seismograms_a, seismograms_p, and seismograms_eps.
    ...
    if(CTD_SEM) then !--------------------------------------m2
    if (SAVE_SEISMOGRAMS_DISPLACEMENT) &
      seismograms_d_m2(:,irec_local,seismo_current) = real(rotation_seismo(:,1)*dxd_m2 + rotation_seismo(:,2)*dyd_m2 &
                                                      + rotation_seismo(:,3)*dzd_m2,kind=CUSTOM_REAL)
    if (SAVE_SEISMOGRAMS_VELOCITY) &
      seismograms_v_m2(:,irec_local,seismo_current) = real(rotation_seismo(:,1)*vxd_m2 + rotation_seismo(:,2)*vyd_m2 &
                                                      + rotation_seismo(:,3)*vzd_m2,kind=CUSTOM_REAL)
    if (SAVE_SEISMOGRAMS_ACCELERATION) &
      seismograms_a_m2(:,irec_local,seismo_current) = real(rotation_seismo(:,1)*axd_m2 + rotation_seismo(:,2)*ayd_m2 &
                                                      + rotation_seismo(:,3)*azd_m2,kind=CUSTOM_REAL)
    endif !-------------------------------------------------m2
