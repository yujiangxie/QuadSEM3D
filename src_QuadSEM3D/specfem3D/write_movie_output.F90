...
#include "config.fh"
  ...
  if (CREATE_SHAKEMAP .and. mod(it,NTSTEP_BETWEEN_FRAMES) == 0) then
...
!        MOVIE_TYPE == 2: uses norm of vector as peak-ground, for all external, outer mesh surfaces
  ...
  ! if (it == NSTEP) call wmo_save_shakemap()
    call wmo_save_shakemap() ! save it for each NTSTEP_BETWEEN_FRAMES, lucas
...
!================================================================
  ...
  ! creates shakemap file
    if (myrank == 0) then
  ! open(unit=IOUT,file=trim(OUTPUT_FILES)//'/shakingdata',status='unknown',action='write',form='unformatted',iostat=ier)
    write(outputname,"('/shakingdata',i6.6)") it
...
!     close(27)
   ! by lucas
   !   write(outputname,"('/proc',i6.6,'_pressure_it',i6.6,'.bin')")myrank,it
   !   open(unit=IOUT,file=trim(OUTPUT_FILES)//trim(outputname),status='REPLACE',action='write',form='unformatted',iostat=ier)
   ! !  if (ier /= 0) stop 'error opening file movie output pressure'
  ...
  !  if (myrank == 0) then
  !  write(IMAIN,*)
  !  write(IMAIN,*) 'lucas to check div_glob outputname = ',trim(outputname)
  !  write(IMAIN,*)
  ...
  !  if (myrank == 0) then
  !  write(IMAIN,*)
  !  write(IMAIN,*) 'lucas to check div outputname = ',trim(outputname)
  !  write(IMAIN,*)
  !  ! flushes file buffer for main output file (IMAIN)
  !  call flush_IMAIN()
  !  endif
  ...
  !  if (myrank == 0) then
  !  write(IMAIN,*)
  !  write(IMAIN,*) 'lucas to check curl_x_it outputname = ',trim(outputname)
  !  write(IMAIN,*)
  !  ! flushes file buffer for main output file (IMAIN)
  !  call flush_IMAIN()
  !  endif
  ...
  ! if (myrank == 0) then
  !  write(IMAIN,*)
 !   write(IMAIN,*) 'lucas to check velocity_x outputname = ',trim(outputname)
 !   write(IMAIN,*)
 !   ! flushes file buffer for main output file (IMAIN)
 !   call flush_IMAIN()
 !   endif
