...
!=====================================================================
  ...
  real(kind=CUSTOM_REAL),dimension(NDIM,NGLLX,NGLLY,NGLLZ),intent(out) :: vector_field_element
    ...
    if (NGLLX == 5 .or. NGLLX == 6 .or. NGLLX == 7) then
      !lucas, to get vector_field_element(3,i,j,k) = (temp1(i,j,k)*xizl + temp2(i,j,k)*etazl + temp3(i,j,k)*gammazl) * rho_invl, where temp1 or temp2 or temp3 = f(scalar_field)
...
!------------------------------------------------------
!
  !lucas, to get vector_field_element(3,i,j,k) = (temp1(i,j,k)*xizl + temp2(i,j,k)*etazl + temp3(i,j,k)*gammazl) * rho_invl, where temp=f(scalar_field).
...
! double loop over GLL points to compute and store gradients
  ...
  if (NGLLX == 5) then
    ...
    call mxm5_single(hprime_xx,m1,field_local,temp1,m2)!lucas, to get temp1(5,25)=hprime_xx(5,5)*field_local(5,25);
    call mxm5_3dmat_single(field_local,m1,hprime_xxT,m1,temp2,NGLLX)!lucas, to get temp2(5,5,5)=field_local(5,5,5)*hprime_xxT(5,5,5);
    call mxm5_single(field_local,m2,hprime_xxT,temp3,m1)!lucas, to get temp3(25,5)=field_local(25,5)*hprime_xxT(5,5);
