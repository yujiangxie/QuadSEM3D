...
!=====================================================================
!
!lucas, to get temp1(i,j,k), temp2(i,j,k), and temp3(i,j,k)
subroutine pml_compute_memory_variables_acoustic(ispec,ispec_CPML,temp1,temp2,temp3, &
                                                 ...
                                                 !lucas,output: temp1, temp2, temp3, 
                                                 !lucas,inoutput: rmemory_dpotential_dxl,rmemory_dpotential_dyl,rmemory_dpotential_dzl,!store the field accel at time step n-1 for second order convolution scheme.
  ...
  do k=1,NGLLZ
    do j=1,NGLLY
      do i=1,NGLLX
        ...
        !---------------------- A6, A7, A8, A9 --------------------------lucas, to get A6, A7, A8, A9, and the coef after them
        call lijk_parameter_computation(deltat, &
                                        kappa_z,d_z,alpha_z,kappa_y,d_y,alpha_y,kappa_x,d_x,alpha_x, & ! lucas ????? why with the order of z, y, x????
                                        CPML_region_local,231,A6,A7,A8,A9, &
                                        coef0_1,coef1_1,coef2_1,coef0_2,coef1_2,coef2_2, &
                                        coef0_3,coef1_3,coef2_3)
                                        !lucas, the first 10 are input, and the last 13 are output.

        rmemory_dpotential_dxl(i,j,k,ispec_CPML,1) = coef0_1 * rmemory_dpotential_dxl(i,j,k,ispec_CPML,1) + &   ! lucas, where rmemory_dpotential_dxl(i,j,k,ispec_CPML,1/2/3) is inout
        ...
        !---------------------- A10,A11,A12,A13 -------------------------lucas, to get A10, A11, A12, A13, and the coef after them
        call lijk_parameter_computation(deltat, &
                                        kappa_x,d_x,alpha_x,kappa_z,d_z,alpha_z,kappa_y,d_y,alpha_y, &
                                        CPML_region_local,132,A10,A11,A12,A13, &
                                        coef0_1,coef1_1,coef2_1,coef0_2,coef1_2,coef2_2, &
        ...
        !---------------------- A14,A15,A16,A17 -------------------------lucas, to get A14, A15, A16, A17, and the coef after them
        call lijk_parameter_computation(deltat, &
                                        kappa_x,d_x,alpha_x,kappa_y,d_y,alpha_y,kappa_z,d_z,alpha_z, &
                                        CPML_region_local,123,A14,A15,A16,A17, &
                                        coef0_1,coef1_1,coef2_1,coef0_2,coef1_2,coef2_2, &
        ...
        ! derivatives, lucas, used the Ai and rmemory_dpotential_dxl(dyl or dzl) determined above.
...
!=====================================================================
!
!lucas, to get displ_x,displ_y,displ_z, and to update rmemory_coupling_ac_el_displ
subroutine pml_compute_memory_variables_acoustic_elastic(ispec_CPML,iface,iglob,i,j,k, &
                                                         displ_x,displ_y,displ_z,displ, &
                                                         num_coupling_ac_el_faces,rmemory_coupling_ac_el_displ)!lucas, out: displ_x,displ_y,displ_z
                                                                                                               !lucas, inout: rmemory_coupling_ac_el_displ
  ! calculates C-PML elastic memory variables and computes stress sigma
  ...
  CPML_region_local = CPML_regions(ispec_CPML) !LUCAS, CPML_regions(:) defined in pml_par
  kappa_x = k_store_x(i,j,k,ispec_CPML)
  kappa_y = k_store_y(i,j,k,ispec_CPML)
  kappa_z = k_store_z(i,j,k,ispec_CPML)
  d_x = d_store_x(i,j,k,ispec_CPML)
  ...
  call lxy_interface_parameter_computation(deltat,kappa_y,d_y,alpha_y,kappa_z,d_z,alpha_z, &
                                           ...
                                           coef0_1,coef1_1,coef2_1,coef0_2,coef1_2,coef2_2) !lucas, to get A_12,A_13,A_14, and the coef after them 
  ! displ_x
...
subroutine lijk_parameter_computation(deltat,kappa_x,d_x,alpha_x,kappa_y,d_y,alpha_y,kappa_z,d_z,alpha_z, &
                                      ...
                                      coef0_1,coef1_1,coef2_1,coef0_2,coef1_2,coef2_2, &
                                      coef0_3,coef1_3,coef2_3)
                                      !lucas, output:A_0,A_1,A_2,A_3,  coef0_1,coef1_1,coef2_1,  coef0_2,coef1_2,coef2_2,  coef0_3,coef1_3,coef2_3
                                      !lucas, input:deltat, kappa_x,d_x,alpha_x,  kappa_y,d_y,alpha_y,  kappa_z,d_z,alpha_z

  use constants, only: CUSTOM_REAL,CPML_X_ONLY,CPML_Y_ONLY,CPML_Z_ONLY, &
                       CPML_XY_ONLY,CPML_XZ_ONLY,CPML_YZ_ONLY,CPML_XYZ
  use pml_par, only: min_distance_between_CPML_parameter !lucas, read from 'external_mesh.bin', and compute at pml_set_local_dampingcoeff.f90 of generate_datasets
  ...
  !lucas, not used, and just for reference.
  !CPML_X_ONLY = 1
  !CPML_Y_ONLY = 2
  !CPML_Z_ONLY = 3
  !CPML_XY_ONLY = 4
  ...
  if (CPML_region_local == CPML_XYZ_TEMP) then
    ...
    if (abs(alpha_x-alpha_y) >= min_distance_between_CPML_parameter .and. &
      ...
      !1.lucas, calculate bar_A_0, bar_A_1, bar_A_2, bar_A_3
      !----------------A0-------------------------
      ...
      !lucas, to get coeff by exp(alpha or beta,deltat)
      call compute_convolution_coef(alpha_x, deltat, coef0_1, coef1_1, coef2_1)!lucas,located in pml_compute_accel_contribution.f90
      call compute_convolution_coef(alpha_y, deltat, coef0_2, coef1_2, coef2_2)!lucas, output: coef0_2, coef1_2, coef2_2.               input:alpha_y, deltat
      call compute_convolution_coef(beta_z, deltat, coef0_3, coef1_3, coef2_3) !lucas, such as: coef0_3 = exp(-beta_z*deltat), 
                                                                               !lucas,          coef1_3 = [1-exp(-beta_z*deltat/2)]/beta_z
                                                                               !lucas,          coef2_3 = [1-exp(-beta_z*deltat/2)]*exp(-beta_z*deltat/2)/beta_z
    ...
    !2.lucas, calculate bar_A_0, bar_A_1, bar_A_2, bar_A_3,   beta_x,beta_y,beta_z
    ...
    !3.lucas, calculate bar_A_0, bar_A_1, bar_A_2, bar_A_3,   beta_x,beta_y,beta_z
    ...
    !4.lucas, calculate bar_A_0, bar_A_1, bar_A_2, bar_A_3,   beta_x,beta_y,beta_z
    ...
    !5.lucas, calculate bar_A_0, bar_A_1, bar_A_2, bar_A_3
    !----------------A0-------------------------
    bar_A_0 = kappa_x
    ...
    !6.lucas, calculate bar_A_0, bar_A_1, bar_A_2, bar_A_3
    !----------------A0-------------------------
    bar_A_0 = kappa_y
    ...
    !7.lucas, calculate bar_A_0, bar_A_1, bar_A_2, bar_A_3
    !----------------A0-------------------------
...
!=====================================================================
!
!lucas, to get A_0,A_1,A_2, coef0_1,and the coef after them 
subroutine lxy_interface_parameter_computation(deltat,kappa_x,d_x,alpha_x,kappa_y,d_y,alpha_y, &
                                      ...
                                      coef0_1,coef1_1,coef2_1,coef0_2,coef1_2,coef2_2)!lucas, out:A_0,A_1,A_2, coef0_1,coef1_1,coef2_1,coef0_2,coef1_2,coef2_2
                                                                                      !lucas, the others are input
