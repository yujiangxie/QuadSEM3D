...
! for acoustic solver
  !lucas, to update potential_dot_dot_acoustic, to get b_absorb_potential, and also write b_absorb_potential in case of adjoint simulations.
  subroutine compute_stacey_acoustic(NSPEC_AB,NGLOB_AB, &
                            potential_dot_dot_acoustic,potential_dot_acoustic, & ! lucas, input: potential_dot_acoustic, inout: potential_dot_dot_acoustic
                            ibool,iphase, &
                            abs_boundary_jacobian2Dw,abs_boundary_ijk,abs_boundary_ispec, & ! lucas, input:abs_boundary_jacobian2Dw, abs_boundary_ijk, abs_boundary_ispec, 
                            num_abs_boundary_faces,rhostore,kappastore,ispec_is_acoustic, & ! lucas, input:num_abs_boundary_faces,rhostore,kappastore,ispec_is_acoustic,
                            SIMULATION_TYPE,SAVE_FORWARD,it,b_reclen_potential, & !lucas, input:SIMULATION_TYPE,SAVE_FORWARD,it,b_reclen_potential
                            b_absorb_potential,b_num_abs_boundary_faces)! lucas, input: b_num_abs_boundary_faces
                                                                        ! lucas, output:b_absorb_potential
...
! adjoint simulations
  ...
  real(kind=CUSTOM_REAL),dimension(NGLLSQUARE,b_num_abs_boundary_faces):: b_absorb_potential !lucas b_num_abs_boundary_faces=num_abs_boundary_faces, set in prepare_timerun()
...
! local parameters
  ...
  if (SIMULATION_TYPE == 1 .and. SAVE_FORWARD) then
    ...
    call write_abs(IOABS_AC,b_absorb_potential,b_reclen_potential,it)!lucas, copy IOABS_AC to b_absorb_potential, i.e., copy fid to buffer either write_abs or read_abs.
  endif
...
! for acoustic solver for back propagation wave field
 !lucas, b_potential_dot_dot_acoustic(iglob) = b_potential_dot_dot_acoustic(iglob) - b_absorb_potential(igll,iface)
  ...
  subroutine compute_stacey_acoustic_backward(NSPEC_AB, &
                            ...
                            b_absorb_potential,b_num_abs_boundary_faces)!lucas, inout:b_potential_dot_dot_acoustic(iglob), and
                                                                        !lucas, to be read in the subroutine:  b_absorb_potential, which is obtained in compute_stacey_acoustic above
