...
!   print *,'update_displacement_scheme() for ELASTIC_SIMULATION'
   call update_displacement_elastic()
   if(CTD_SEM) then !-----m2
     call update_displacement_elastic_m2()
     if(compute_approx_Hessian) then !----------m1
       call update_displacement_elastic_m1()
     endif !------------------------------------m1
   endif !----------------m2
  endif
             ...
             do j = 1, NGLLY
                do k = 1, NGLLZ
                   ...
                   PML_potential_acoustic_old(i,j,k,ispec_cpml) = potential_acoustic(iglob) &
                       ...
                       + deltatsqover2 * (1._CUSTOM_REAL - theta) * potential_dot_dot_acoustic(iglob) ! lucas, to get PML_potential_acoustic_old
                enddo
             enddo
          enddo
       enddo
            ...
            do j = 1, NGLLY
               do k = 1, NGLLZ
                  ...
                  PML_potential_acoustic_new(i,j,k,ispec_cpml) = potential_acoustic(iglob) &
                       + deltatover2 * (1._CUSTOM_REAL - 2.0_CUSTOM_REAL*theta) * potential_dot_acoustic(iglob) ! lucas, to get PML_potential_acoustic_new
               enddo
            enddo
         enddo
      enddo
      ...
      if (PML_CONDITIONS) then
        if (nglob_interface_PML_acoustic > 0) then
          call read_potential_on_pml_interface(b_potential_dot_dot_acoustic,b_potential_dot_acoustic,b_potential_acoustic, &
                                               nglob_interface_PML_acoustic,b_PML_potential,b_reclen_PML_potential)!  lucas, the first three are output, and the last three are input?, and keep here since somewhere should have the save so that we can read? 
        endif
      endif

      ! lucas, the same as above 1111 but with b
...
!
  ...
  if (.not. GPU_MODE) then ! lucas ------------------------------------------CPU-------------------------------
    ! wavefields on CPU
      ...
      if (PML_CONDITIONS) then
        if (nglob_interface_PML_elastic > 0) then
          call read_field_on_pml_interface(b_accel,b_veloc,b_displ,nglob_interface_PML_elastic, &
                                           b_PML_field,b_reclen_PML_field) !  lucas, the first 3 are output, and the last 3 are input?, and keep here since somewhere should have the save so that we can read? 
        endif
      endif
...
!--------------------------------------------------------------------m2 ---------------------------------
  subroutine update_displacement_elastic_m2() !lucas, CTD_SEM
...
!        enddo
!    endif
    displ_m2(:,:) = displ_m2(:,:) + deltat * veloc_m2(:,:) + deltatsqover2 * accel_m2(:,:)
    veloc_m2(:,:) = veloc_m2(:,:) + deltatover2 * accel_m2(:,:)
    if (SIMULATION_TYPE /= 1) accel_adj_coupling_m2(:,:) = accel_m2(:,:)
    accel_m2(:,:) = 0._CUSTOM_REAL
!    if (PML_CONDITIONS .and. NSPEC_CPML > 0) then
!        do ispec_cpml=1,NSPEC_CPML
!           ispec = CPML_to_spec(ispec_cpml)
!           do k = 1, NGLLZ
...
!        endif
!      endif
      b_displ_m2(:,:) = b_displ_m2(:,:) + b_deltat * b_veloc_m2(:,:) + b_deltatsqover2 * b_accel_m2(:,:)
  ...
  end subroutine update_displacement_elastic_m2


!--------------------------------------------------------------------m1 ---------------------------------
  subroutine update_displacement_elastic_m1() !lucas, CTD_SEM, only work with SIMULATION_TYPE == 3
...
!        enddo
!    endif
    displ_m1(:,:) = displ_m1(:,:) + deltat * veloc_m1(:,:) + deltatsqover2 * accel_m1(:,:)
    veloc_m1(:,:) = veloc_m1(:,:) + deltatover2 * accel_m1(:,:)
    if (SIMULATION_TYPE /= 1) accel_adj_coupling_m1(:,:) = accel_m1(:,:)
    accel_m1(:,:) = 0._CUSTOM_REAL
!    if (PML_CONDITIONS .and. NSPEC_CPML > 0) then
!        do ispec_cpml=1,NSPEC_CPML
!           ispec = CPML_to_spec(ispec_cpml)
!           do k = 1, NGLLZ
...
!        endif
!      endif
!      b_displ(:,:) = b_displ(:,:) + b_deltat * b_veloc(:,:) + b_deltatsqover2 * b_accel(:,:) !lucas, for reconstrcued fields for m1 (the orignal model)
!      b_veloc(:,:) = b_veloc(:,:) + b_deltatover2 * b_accel(:,:)
!      b_accel(:,:) = 0._CUSTOM_REAL
!    endif
  ...
  end subroutine update_displacement_elastic_m1
