...
! density
  ...
  ! isotropic
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: kappastore_m2,mustore_m2 ! CTD_SEM
! density
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rhostore_m2 !CTD_SEM
...
! seismograms
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: seismograms_d_m2,seismograms_v_m2,seismograms_a_m2 !CTD_SEM
...
! arrays for elemental computations inside a given spectral element
  ...
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ) :: tempy1_m2,tempy2_m2,tempy3_m2
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ) :: tempz1_m2,tempz2_m2,tempz3_m2
  ...
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ) :: tempy1_m1,tempy2_m1,tempy3_m1
  real(kind=CUSTOM_REAL), dimension(NGLLX,NGLLY,NGLLZ) :: tempz1_m1,tempz2_m1,tempz3_m1
...
! maximum speed in velocity model
  ...
  real(kind=CUSTOM_REAL):: model_speed_max_m2 !lucas, CTD_SEM
...
! ADJOINT parameters
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: source_adjoint_m2 ! CTD_SEM
...
! parameter module for elastic solver
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:,:,:), allocatable :: factor_common_m2,factor_common_kappa_m2 !lucas CTD_SEM
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: tau_sigma
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: tau_sigma_m2 !CTD_SEM
  real(kind=CUSTOM_REAL) :: min_resolved_period
  real(kind=CUSTOM_REAL) :: min_resolved_period_m2 !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: &
    alphaval,betaval,gammaval
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: &  !lucas, CTD_SEM
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: epsilon_trace_over_3_m2 ! CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: epsilon_trace_over_3_m1 ! CTD_SEM

  real(kind=CUSTOM_REAL), dimension(:,:,:,:,:), allocatable :: R_trace_m2,R_xx_m2,R_yy_m2,R_xy_m2,R_xz_m2,R_yz_m2 ! CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: &
    epsilondev_trace_m2,epsilondev_xx_m2,epsilondev_yy_m2,epsilondev_xy_m2,epsilondev_xz_m2,epsilondev_yz_m2  ! CTD_SEM

  real(kind=CUSTOM_REAL), dimension(:,:,:,:,:), allocatable :: R_trace_m1,R_xx_m1,R_yy_m1,R_xy_m1,R_xz_m1,R_yz_m1 ! CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: &
    epsilondev_trace_m1,epsilondev_xx_m1,epsilondev_yy_m1,epsilondev_xy_m1,epsilondev_xz_m1,epsilondev_yz_m1  ! CTD_SEM
...
! displacement, velocity, acceleration
  ...
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: displ_m2,veloc_m2,accel_m2 !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: accel_adj_coupling_m2 !lucas, CTD_SEM

  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: displ_m1,veloc_m1,accel_m1 !lucas, CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: accel_adj_coupling_m1 !lucas, CTD_SEM

! mass matrix
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: rmass
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: rmass_m2 !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: rmass_m1 !CTD_SEM

! Stacey
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: rmassx,rmassy,rmassz
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_vp,rho_vs

  real(kind=CUSTOM_REAL), dimension(:), allocatable :: rmassx_m2,rmassy_m2,rmassz_m2 !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:), allocatable :: rmassx_m1,rmassy_m1,rmassz_m1 !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_vp_m2,rho_vs_m2 !CTD_SEM
...
! ADJOINT elastic
  ...
  real(kind=CUSTOM_REAL), dimension(:,:), allocatable :: b_displ_m2, b_veloc_m2, b_accel_m2 ! CTD_SEM

  ! backward attenuation arrays
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: &
    b_alphaval, b_betaval, b_gammaval
  real(kind=CUSTOM_REAL), dimension(N_SLS) :: & !CTD_SEM
    b_alphaval_m2, b_betaval_m2, b_gammaval_m2

  real(kind=CUSTOM_REAL), dimension(:,:,:,:,:), allocatable :: &
    b_R_trace,b_R_xx,b_R_yy,b_R_xy,b_R_xz,b_R_yz

  real(kind=CUSTOM_REAL), dimension(:,:,:,:,:), allocatable :: & !CTD_SEM
    b_R_trace_m2,b_R_xx_m2,b_R_yy_m2,b_R_xy_m2,b_R_xz_m2,b_R_yz_m2
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: & ! CTD_SEM
  b_epsilondev_trace_m2,b_epsilondev_xx_m2,b_epsilondev_yy_m2,b_epsilondev_xy_m2,b_epsilondev_xz_m2,b_epsilondev_yz_m2
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: b_epsilon_trace_over_3_m2

  ! adjoint kernels
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl, mu_kl, kappa_kl
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl_m2, mu_kl_m2, kappa_kl_m2 ! CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl_Ha, mu_kl_Ha, kappa_kl_Ha !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl_Hbm, mu_kl_Hbm, kappa_kl_Hbm !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl_Hbs, mu_kl_Hbs, kappa_kl_Hbs !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl_Hc, mu_kl_Hc, kappa_kl_Hc !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl_Habc, mu_kl_Habc, kappa_kl_Habc !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: rho_kl_Hdiff, mu_kl_Hdiff, kappa_kl_Hdiff !CTD_SEM
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: b_absorb_field_m2 !CTD_SEM
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable :: b_absorb_field_m1 !CTD_SEM
  integer :: b_reclen_field
!  integer :: b_reclen_field_m2 !CTD_SEM, this does not need anymore since the same for m1 and m2
