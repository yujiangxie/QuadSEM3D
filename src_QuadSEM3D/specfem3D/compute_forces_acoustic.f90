...
! computes forces for acoustic elements
  ...
  ! loop over spectral elements, lucas!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  do ispec_p = 1,num_elements
  !1.lucas
    ispec = phase_ispec_inner_acoustic(ispec_p,iphase)! lucas, to get the ispec in case of iphase
...
! and duplicate the content of the loop, using one for each case of the "if" test.
  ...
  if (NGLLX == 5) then
    ...
    call mxm5_single(hprime_xx,m1,chi_elem,temp1,m2)! lucas, temp1(5,25) = hprime_xx(5,5)*chi_elem(5,25). where m1=5, m2=25
    call mxm5_3dmat_single(chi_elem,m1,hprime_xxT,m1,temp2,NGLLX)! lucas, temp2(5,5,5) = chi_elem(5,5,5)*hprime_xxT(5,5)
    call mxm5_single(chi_elem,m2,hprime_xxT,temp3,m1)!lucas, temp3(25,5) = chi_elem(25,5)*hprime_xxT(5,5)
...
!
  !4.lucas
  if (is_CPML(ispec) .and. .not. backward_simulation) then ! lucas, CPML and forward-----------------------------------------------------------------
    ...
    do k = 1,NGLLZ
      do j = 1,NGLLY
        do i = 1,NGLLX
          ...
          chi_elem_new(i,j,k) = PML_potential_acoustic_new(i,j,k,ispec_CPML)!lucas, where new is upadted.
        enddo
      enddo
    enddo
        ...
        ! computes 3. matrix multiplication for temp3
    if (NGLLX == 5) then
        call mxm5_single_two_arrays_at_a_time(hprime_xx,m1,chi_elem_old,temp1_old,m2,chi_elem_new,temp1_new)! lucas, temp1_old(5,25) = hprime_xx(5,5)*chi_elem_old(5,25);
                                                                                                            ! lucas, temp1_new(5,25) = hprime_xx(5,5)*chi_elem_new(5,25);

        call mxm5_3dmat_single_two_arrays_at_a_time(chi_elem_old,m1,hprime_xxT,m1,temp2_old,NGLLX,chi_elem_new,temp1_new)! lucas, temp2_old(5,5,5) = chi_elem_old(5,5,5)*hprime_xxT(5,5);
                                                                                                                         ! lucas, temp1_new(5,5,5) = chi_elem_new(5,5,5)*hprime_xxT(5,5);?

        call mxm5_single_two_arrays_at_a_time(chi_elem_old,m2,hprime_xxT,temp3_old,m1,chi_elem_new,temp1_new)! lucas,temp3_old(25,5) = chi_elem_old(25,5)*hprime_xxT(5,5);
                                                                                                             ! lucas,temp1_new(25,5) =chi_elem_old(25,5)*chi_elem_new(5,5);?
    else if (NGLLX == 6) then
      ...
      do j=1,NGLLY
        do i=1,NGLLX
              ...
              rho_invl = 1.0_CUSTOM_REAL / rhostore(i,j,k,ispec)
                ...
                ! derivatives of potential
                dpotentialdxl = xixl*temp1(i,j,k) + etaxl*temp2(i,j,k) + gammaxl*temp3(i,j,k)!lucas, dp/dx
                dpotentialdyl = xiyl*temp1(i,j,k) + etayl*temp2(i,j,k) + gammayl*temp3(i,j,k)!lucas, dp/dy
                dpotentialdzl = xizl*temp1(i,j,k) + etazl*temp2(i,j,k) + gammazl*temp3(i,j,k)!lucas, dp/dz

                PML_dpotential_dxl_old(i,j,k) = xixl*temp1_old(i,j,k) + etaxl*temp2_old(i,j,k) + gammaxl*temp3_old(i,j,k)!lucas, PML_dp/dx_old
                PML_dpotential_dyl_old(i,j,k) = xiyl*temp1_old(i,j,k) + etayl*temp2_old(i,j,k) + gammayl*temp3_old(i,j,k)!lucas, PML_dp/dy_old
                PML_dpotential_dzl_old(i,j,k) = xizl*temp1_old(i,j,k) + etazl*temp2_old(i,j,k) + gammazl*temp3_old(i,j,k)!lucas, PML_dp/dz_old

                PML_dpotential_dxl_new(i,j,k) = xixl*temp1_new(i,j,k) + etaxl*temp2_new(i,j,k) + gammaxl*temp3_new(i,j,k)!lucas, PML_dp/dx_new, where is the temp2_new and temp3_new?
                PML_dpotential_dyl_new(i,j,k) = xiyl*temp1_new(i,j,k) + etayl*temp2_new(i,j,k) + gammayl*temp3_new(i,j,k)!lucas, PML_dp/dy_new
                PML_dpotential_dzl_new(i,j,k) = xizl*temp1_new(i,j,k) + etazl*temp2_new(i,j,k) + gammazl*temp3_new(i,j,k)!lucas, PML_dp/dz_new
    ...
    else ! regular element, lucas-------------------to get: PML_dpotential_dxl, PML_dpotential_dxl_old, PML_dpotential_dxl_new, temp1,temp2,temp3
...
! %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  !5.lucas
  else ! no PML in this element, lucas-------------------no PML or back forward------------------to get temp1 temp2 temp3
  ...
  !6.lucas
    if (is_CPML(ispec) .and. .not. backward_simulation) then
        ...
        call pml_compute_memory_variables_acoustic(ispec,ispec_CPML,temp1,temp2,temp3, &
                                                   ...
                                                   PML_dpotential_dxl,PML_dpotential_dyl,PML_dpotential_dzl, &
                                                   PML_dpotential_dxl_old,PML_dpotential_dyl_old,PML_dpotential_dzl_old, &
                                                   PML_dpotential_dxl_new,PML_dpotential_dyl_new,PML_dpotential_dzl_new)!lucas, to get temp1,temp2,temp3
                                                   !lucas, inoutput:rmemory_dpotential_dxl,   rmemory_dpotential_dyl,  rmemory_dpotential_dzl(NGLLX,NGLLY,NGLLZ,NSPEC_CPML,3)
                                                   !lucas, where the rmemory_dpotential_dxl etc. store the field accel at time step n-1 for second order convolution scheme.

        ! calculates contribution from each C-PML element to update acceleration
        call pml_compute_accel_contribution_acoustic(ispec,ispec_CPML,potential_acoustic, &
                                                     potential_dot_acoustic,rmemory_potential_acoustic)!lucas, to get potential_dot_dot_acoustic_CPML(i,j,k)
    endif
...
!
  ...
  !7.lucas, to get newtemp1, newtemp2, newtemp3
  if (NGLLX == 5) then

    call mxm5_single(hprimewgll_xxT,m1,temp1,newtemp1,m2)!lucas, newtemp1(5,25) = hprimewgll_xxT(5,5)*temp1(5,25). where m1=5, m2=25

    call mxm5_3dmat_single(temp2,m1,hprimewgll_xx,m1,newtemp2,NGLLX)!lucas,newtemp2(5,5,5) = temp2(5,5,5)*hprimewgll_xx(5,5)

    call mxm5_single(temp3,m2,hprimewgll_xx,newtemp3,m1)!lucas, newtemp3(25,5)=temp3(25,5)*hprimewgll_xx(5,5)
  ...
  !8.lucas, to get potential_dot_dot_acoustic(iglob)
    ! sum contributions from each element to the global values
    ! this loop will not fully vectorize because it contains a dependency (through indirect addressing with array ibool())
    do k = 1,NGLLZ
      do j = 1,NGLLY
...
!
  ...
  enddo ! end of loop over all spectral elements!lucas!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
