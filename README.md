QuadSEM3D is the 3D version of QuadSEM (https://github.com/yujiangxie/QuadSEM), which is based on the SPECFEM3D package. I constructed QuadSEM3D by modifying the src folder of SPECFEM3D. In other words, QuadSEM3D solves four wave equations simultaneously, whereas SPECFEM3D solves one (forward) or two wave equations (adjoint, e.g., the on-the-fly) at a time.

One can use QuadSEM3D in essentially the same way as SPECFEM3D; I have kept it as similar to SPECFEM3D as possible. The key difference is that QuadSEM3D works with two models simultaneously to compute the full Hessian kernels on the fly (for elastic cases), which can substantially reduce storage and I/O costs and make the use of full Hessian kernels feasible for large-scale simulations and imaging.
